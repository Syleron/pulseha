...
| TC-1 | #84 the two-available-node tie-break answers "should *I* win" to a question asked about another candidate | **Fixed 2026-08-25 (END-2325), NOT VERIFIED LIVE — and the severity originally filed was too high, corrected here.** `initiateNodeStatusVote(nodeID, newStatus)` takes `nodeID` as the subject of the vote; its `availableNodes == 2` branch ignored it entirely and returned `localNodeID < otherNodeID`. That answers a different question whenever the candidate is not the local node, which via `attemptVotingElection` it frequently is not — `selectBestCandidate` gives the local node only `+5` against a score built from status (50/25), latency (up to 20) and recency (10). **The property actually broken is agreement, not permissiveness:** two nodes running this concurrently for the same candidate computed opposite verdicts, which is precisely what a tie-break with no majority behind it must never do. Same lesson as the config tiebreak, which had to become origin-versus-origin rather than sender-versus-receiver. Now decided on the subject — `nodeID == lowest(available)` — so every node reaches the same verdict about the same candidate; a subject that is not one of the two contenders is outside the tie this rule breaks and is allowed rather than refused, with `confirmPeerReleasedIPs` still gating the promotion downstream. **Reachable only in a DEGRADED cluster of three or more**, since `availableNodes` counts Active and Passive: the Active-failed-two-Passives-remain case, or two of four Unknown. A genuine two-node cluster never arrives here and must not — see ADR-0002. **Severity correction:** the original report claimed this blocks failover in degraded clusters. It does not, because `handlePartialFailure` — the one caller where `!voteResult` returns and abandons the promotion — **has no production callers at all** (#85). The only live caller is `attemptVotingElection`, and `electNewActiveNode` calls `tryForcePromote(bestCandidate)` in *both* branches of its result, so the vote changes almost nothing there. The fix is still right, and cheap, but this was a latent correctness bug rather than an outage. Tests `internal/membership/two_node_tiebreak_test.go`; the old rule observed failing three of five cases, decisively on `verdict on node-b depends on who asked: self=false other=true`. `handlePartialFailure`'s candidate selection was also made deterministic — it took the first Passive the member map yielded, and Go randomises map iteration, so it asked the vote about a coin flip **Reachability covered 2026-09-03, and it needed a new stub capability.** The original tests call `initiateNodeStatusVote` directly, so they pin the decision but prove nothing about a degraded cluster ever arriving at it -- which is why this branch was once mistaken for dead code, and what the ticket asked for. Reachability is not obvious because the two gates count different things: `attemptVotingElection` needs three or more members that are Passive **or Unknown**, while the tie-break needs exactly two that are Active **or Passive**. `internal/membership/degraded_election_test.go` builds both shapes that satisfy both at once -- the three-node Active-failed-two-Passives-remain case and two-of-four-Unknown -- drives `attemptVotingElection`, and asserts on the `Exactly 2 nodes available` line so it fails loudly if the branch is not entered rather than passing vacuously. **The trap:** `stubServer.GetQuorumManager()` returned a hard-coded nil, and `attemptVotingElection` skips the vote entirely when it is nil, so any such test silently measured the fallback path instead. The stub now carries an optional real `QuorumManager` (nil by default, so every existing test is unaffected) and `TestTheTiebreakIsSkippedWithoutAQuorumManager` records why. The three-node and four-node tests compare verdicts across every asker, and were **observed to fail against the restored pre-END-2325 rule** -- the actual defect, through the path production takes. A fourth test pins ADR-0002 at the election level, where nothing had: a genuine two-node cluster must fall through to direct promotion and never enter the branch, since one node would then lose on node ID alone and across a partition that can be the half still serving traffic. It fails when `attemptVotingElection`'s gate is dropped from three to two. **Still NOT VERIFIED LIVE:** the ticket wants a degraded whitecrane (one node down, Active failed) and a look at whether the promotion the election chose is the one that happens. That second question is largely answered by the severity correction above -- `electNewActiveNode` calls `tryForcePromote(bestCandidate)` in both branches of the vote result, so the verdict changes almost nothing there -- but neither has been watched on real hardware. |
| TC-1 | #85 `handlePartialFailure` was dead code containing three self-deadlocks, and hunting its orphans found a fourth on a live path | **Dead code deleted and the live deadlock fixed, 2026-08-25.** Found by writing the first test that has ever called this function. `Member` embeds a plain `sync.RWMutex`, which is not reentrant, and three sites took it while it was already held: (a) the promotion path re-locked `member` to clear `ActiveIPs` when it had been locked at the top of the function and re-locked after the vote; (b) `member.RemoveIPs(failedIPs)` was called with that same lock held, and `RemoveIPs` takes it — so this arm deadlocked on **every** call in **every** mode, not only the promotion one; (c) inside `Member.RemoveIPs` itself, which held the lock across `BringDownIPs`, which takes it again on both the local and the remote branch. Each fix is small — drop the redundant re-lock, release before the call that locks internally, hoist `IsLocal` out and release before the bring-down — and each was mutation-verified by reverting it and watching the test time out. **This runs on the health-check goroutine, which holds `h.Lock()` for the whole pass**, so any of the three wedges the health checker with a write lock held: #56's and #79's shape a third and fourth time, and the fourth, fifth and sixth entries in this branch's running tally of non-reentrant `RWMutex` traps (#46, `RebalanceCluster`, `hasQuorumLocked`, #32). **The function has no callers.** Nothing in the tree reaches `handlePartialFailure`; `Member.RemoveIPs` in turn has exactly one caller, which is `handlePartialFailure`. So this whole subsystem has never executed, which is why three unconditional deadlocks sat in it undetected. **Deleted rather than repaired**, on #68's precedent that a trap should be removed rather than left available: 150 lines of never-run code with three deadlocks in it is exactly what the next person to wire up partial-IP-failure handling would inherit. `Member.RemoveIPs` went with it — `handlePartialFailure` was its only caller — along with a now-unused `utils` import. `initiateIPRedistributionVote`, `initiateNodeStatusVote` and `MemberList.RedistributeIPs` all keep live callers elsewhere and stay. Net -305 lines. **The find that justifies the whole exercise came from checking what else was orphaned:** `Member.EnterMaintenance` has the identical defect — `m.Lock()` with a deferred unlock, then `BringDownIPs`, which takes the same lock — and it is **live**, reached from the `SetMaintenance` RPC behind `pulsectl node maintenance`. It deadlocks whenever the target is Active and holding addresses, which is the case the command exists for; its own doc comment says "If the node is currently active, a failover is triggered first". The calling goroutine then holds the member lock forever, so anything else touching that member wedges behind it. Fixed by snapshotting the addresses, releasing the lock across the bring-down, and re-taking it to mark the transition — the documented ordering (addresses down before the status is marked) is preserved, and the status is re-checked on the second acquisition because it can move while the addresses are coming down. Tests `internal/membership/maintenance_test.go`, five cases, each under a 5s watchdog so a regression reports as a failing assertion rather than as the package timing out with a stack dump; the original observed failing on `EnterMaintenance did not return within 5s`. **Six non-reentrant `RWMutex` traps in this branch now** (#46, `RebalanceCluster`, `hasQuorumLocked`, #32, and these), which is the argument for the lock discipline being the thing to fix rather than each site |
| TC-9 | #86 the IP monitor's initialisation reads a member's Status unlocked, which #83's fix made concurrently reachable | **Fixed 2026-08-25, caught by `-race`, and introduced by this branch.** `initializeExpectedIPs` read `localMember.Status` four times without the member lock, while `ConfigSync` writes it holding that lock. The read is older than this branch, but #83 moved `IPMonitor.Start` out of `Server.Start` — where it ran before the cluster listener was up — and into `startHealthChecker`, so a sync can now land while the monitor is initialising. **Intermittent, and it passed three earlier full `-race` runs before failing**, which is the same lesson #71 records: `-race` finds these only when something actually drives both paths at once, so a clean run is not evidence. Snapshotted once under the member lock and the four reads take the snapshot; taking a member lock from inside the monitor is the established direction, `deriveExpectedIPs` already does it through `GetActiveIPs()`. Verified with the failing test run eight times and the full tree three times, all clean. **The general shape belongs to END-2339:** a field guarded by a lock that only some readers take, with nothing making the requirement visible at the read site |
| TC-3 | #87 `config.Nodes` is read unlocked by the health-check tick while the join handler writes it, so `make testrace` is intermittently red | **Fixed 2026-08-25. Pre-existing — not introduced by this branch, and confirmed so.** `Member.IsLocal()` goes to `m.config.GetLocalNodeUUID()`, whose `ClusterCheck()` reads `len(c.Nodes)`; the join handler writes `s.config.Nodes[nodeID]` under `s.config.Lock()`, which that read never takes. `-race` reports it as a map read in `performHealthChecks` against a map write at the join. **#71's shape one level in:** that defect synchronised the config *pointer* and explicitly left the contents, and this is a contents race on the one map a join mutates. **Reproduced on `origin/dev` in 3 of 6 runs**, so it predates every change here — checked precisely because #83 moved the health checker's start to cluster-create/join time and made this branch the obvious suspect. On this branch it did not fire across five consecutive full `-race` runs, which says nothing except that it is intermittent. **The cost is CI, not the daemon:** #74 wired `./tests/...` into both workflows under `-race`, so this makes the suite flaky at roughly a third of runs — and a suite that fails a third of the time is one nobody reads, which is how #79 stayed hidden. **Fix:** `ClusterCheck` and `GetLocalNodeUUID` take the config lock, which is the one the join handler already writes under. **The interesting part is why that is three lines and not one.** `Config` embeds a plain `sync.Mutex`, and `GetLocalNodeUUID` calls `ClusterCheck` — so locking both naively self-deadlocks. Worse, `Validate` also calls `ClusterCheck`, and `Validate` runs from `Load` and from `saveLocked` **with the lock already held**, so an exported `ClusterCheck` that locks would deadlock the config load path on startup. Split into `clusterCheckLocked()` for holders and a locking `ClusterCheck()` for everyone else — the `Locked` suffix convention earning its keep rather than being decorative. Checked every holder of the config lock, directly and one call deep, before touching it; that is what turned up the `Validate` path. **Verified by repetition, since the failure is intermittent:** the racing test 10/10 clean against 3-of-6 racing on `origin/dev`, plus five clean full-tree `-race` passes. **Left alone deliberately:** `UpdateValue` mutates `c.Pulse` and calls `Validate` with no lock at all, and `GetLocalNode` reads `c.Nodes[uuid]` unlocked after its checks. Both are pre-existing and neither is what CI trips on; they belong with the rest of the lock-discipline work in END-2339 |
| TC-1 | #88 `TestNodesAgreeOnTheStatusTheyPublish` asserts agreement at an instant, so it fails about a third of CI runs | **Fixed 2026-08-25, pre-existing.** The final loop compared `node1.ReportedStatus(target)` against `node2.ReportedStatus(target)` directly — two sequential RPCs against a cluster whose statuses are still moving, and these tests drive a **1ms** health-check interval, so an election lands between the two reads and they straddle it. Not disagreement between the nodes: a race in the assertion. What the test means is that the two *converge* on one answer, which is a property with a deadline rather than an instant — exactly the lesson `wait_test.go` already carries at the top of the file for `requireMemberStatus`, whose comment says the old fixed sleeps "asserted at one arbitrary instant". This loop was the one assertion in the package still doing it. **Measured rather than assumed**, containerised on Linux, 15 runs each: `origin/dev` unmodified **5 fail**, `origin/dev` with the polling assertion **0 fail**, this branch unmodified **6 fail**, this branch with it **0 of 20**. So the flake predates every change here and is cured by the assertion alone. Fixed with `requireAgreedStatus`, which polls both observers to `statusSettleTimeout` and reports what each last said. **Two traps in measuring this, both of which produced a wrong answer first.** The tests skip themselves off Linux, so a green `go test` on macOS is a skip, not a pass. And `rpc/*.pb.go` is generated and **untracked** — only `rpc/server.proto` is in git — so a clean `git archive` export of `dev` fails to build, which reports as `FAIL` and is easily misread as the test failing; an earlier reading of "dev fails 15/15" was that, not a result |
| TC-1 | #89 the non-reentrant mutex discipline is unenforced, so a deadlock is silent | **Fixed 2026-09-02 (END-2339), VERIFIED LIVE 2026-09-03 — on the docker rig and on a lab appliance under systemd.** Nine deadlocks came from this: #32, #46, #56, #85, #87, plus `RebalanceCluster`, `hasQuorumLocked`, `Server.PromoteNode` and `Member.RemoveIPs`. **Every one was a method calling a locking sibling on its own receiver, inside its own file** — the constraint was already local and already reviewable, and was still missed nine times, which is what rules out the ticket's proposed fix of un-embedding the mutex: it moves `Lock()` off a public surface that was never where these lived. What makes them worth catching is the failure mode. #56 wedged the health-check goroutine *while holding the write lock*: no node promoted, a 287-address group down, and `ACTIVE_CHECK: Starting active node failure check` logged **0 times in six minutes**. **Fix: `packages/pulselock`, drop-in `Mutex`/`RWMutex` that embed and wrap the `sync` types with matching method sets**, so all 20 declarations converted with **zero call-site changes** and the ~330 existing acquisitions compiling untouched. **Two mechanisms, because exactness is not affordable.** Detecting reentrancy exactly needs goroutine identity, and `runtime.Stack` costs **~1710ns against ~16.5ns** for the uncontended lock it would guard — about **100x** (benchmarks committed in `pulselock_bench_test.go`, since the design turns on them). So under `go test` identity is tracked and a reentrant acquisition **panics immediately** naming what was already held; in the daemon an acquisition still blocked after 30s dumps every goroutine's stack to stderr and **keeps waiting**, so behaviour is identical to an uninstrumented mutex — the daemon wedges exactly as it does today, it just says so first. The production half is a *wedge* detector rather than a reentrancy detector: less precise, and it catches strictly more, since all nine shapes block forever and so do lock-ordering cycles between two objects (#79's class), which no reentrancy check can see. **Three details are load-bearing.** Reads gate on a `writeHeld` flag so an uncontended `RLock` pays one atomic load — an earlier draft used `TryRLock` as the read fast path and measured **3x** a bare `RWMutex`, because a CAS retries under read contention where a plain Add does not. The diagnostic goes to `os.Stderr`, **not the project logger**, because #33/#61 record a Debug line in `packages/network` that could not reach the journal at any `logging_level` and made its own fix unverifiable; under `Type=simple` stderr reaches `journalctl` unconditionally. And the watchdog dumps *all* goroutines, because a `time.AfterFunc` callback runs on the timer's goroutine so a single-goroutine trace would show the timer, while capturing the caller's stack up front would put the ~1710ns on every contended acquisition. **28 tests, 14 mutations, all caught** — the four panic mutations as a *timeout*, since a deadlock hangs the package rather than failing it. One escaped the first pass and the fix was to the test: `TestAnUncontendedAcquisitionArmsNoWatchdog` ran its reads before any write, so a `writeHeld` never cleared had nothing to show; reordered it catches it at 1000 armed watchdogs against 0. **Proven against the real code**: reintroducing #85 (`EnterMaintenance` holding the lock across `BringDownIPs`), #87 (`GetLocalNodeUUID` calling the locking `ClusterCheck`) and the same shape in `Validate` each now **panic** where they previously hung. **It caught a live one during END-2339 itself** — see #98. **Residual, deliberate:** nothing about lock *ordering* under `go test` (the panic path is reentrancy only), nothing about unsynchronised access (`-race`'s job, noting #71 records `-race` missing one *structurally*), and read-then-read on an `RWMutex` is reported but not fatal, since it deadlocks only when a writer queues between the two acquisitions. **Live check: done, and it is the step that mattered.** A reentrant acquisition was injected into `Server.Token` — `s.Lock()` held, then `s.GetClusterEpoch()`, which takes `s.RLock()`: `Server.PromoteNode`'s historical shape exactly — and triggered with `pulsectl cluster token`. On the three-node docker rig and again on lab appliance `MC-LB-3-node-4` under systemd, the diagnostic appeared after `30.001s` reading `a pulselock.RWMutex read acquisition has been blocked for 30.001065881s — the daemon is probably deadlocked`, followed by the full goroutine dump. **The dump is diagnosable, which is the part worth checking rather than assuming:** following the header's own instruction leads to `pulselock.(*RWMutex).RLock` at `pulselock.go:400`, `Server.GetClusterEpoch` at `server.go:7034` and `Server.Token` at `server.go:4780`, with file and line for both acquisitions, and the same mutex address recurring in the other goroutines queued behind the wedge. **The decisive detail is the log level.** The appliance was at `logging_level: info` with **zero** `DEBU` lines in the journal for the whole window, and the report arrived anyway — because it bypasses the project logger and writes to `os.Stderr`, which systemd captures for `Type=simple`. That is #61 closed rather than repeated: the `packages/network` Debug line that could not reach the journal at any `logging_level` is exactly what this design avoids. **The behavioural promises were checked too, not just the output.** The daemon kept waiting rather than dying — `running=true`, `restarts=0`, `exitcode=0`, no panic — exactly **one** report was emitted despite several goroutines queueing on the same lock, and `pulsectl status` blocked as well, which is the cascade an uninstrumented mutex would also produce. Behaviour is unchanged; only the silence is. The injected build was deployed to one Passive node holding no addresses, rolled back immediately afterwards from the copy kept at `/usr/local/sbin/pulseha.v4.0.2`, and the cluster returned to `online` with all four nodes on the released build. The injection branch was deleted unpushed |
| TC-3 | #90 `./tests/unit/...` runs in no CI target, hiding a deadlock regression test | **Fixed 2026-09-02 (END-2339).** `make test`/`testrace` cover `./internal/...`, `./cmd/...`, `./packages/...`; `integration-test` covers `./tests/integration/...`. Nothing covered `./tests/unit/...`, so its fifteen tests had **never run in CI**. #74's row claims it "wired `./tests/...` into both workflows" — it did not: it wired the integration tree only, and the unit tree under the same parent was left out. The tests it was hiding are the ones this repo can least afford to leave unrun: `TestCreateClusterReturnsWithoutDeadlock`, `TestClusterInit_PostCreate_Rejects_CreateClusterAndInitiateJoin` and `TestClusterInit_HandleNodeJoinBlocksDuringCreateCluster` (the TOCTOU guards that stop two nodes both activating as "first node" — dual-active in active-passive), `TestReconfigureConcurrent_NoBindRace`, and `TestMemberList_RedistributeIPs`, which is #46's area. **A deadlock regression test that never executes is worse than none, because it reads as coverage.** They all pass as they stand — 2.6s plain, 4.0s under `-race` — so this cost nothing and was hiding no failure, which is worth stating: the tests were correct and merely unwired. `./tests/unit/...` rather than `./tests/...`, because the latter re-runs the integration suite that `quiet-integration-test` already runs with its own 2m budget and `PULSEHA_TEST_LOGLEVEL`; `./tests/testutils` has no test functions **What turning it on actually caught, 2026-09-03:** the PR went red on Linux only, and it took the race detector on a Linux container to see any of it -- a macOS `-race` run cannot compile `ip_monitor_linux.go` at all, so a whole file of unsynchronised reads had never been checked. Five distinct races, in two families. **Unlocked member reads:** `enforceExpectations` read `member.Status` nine times unlocked, and the netlink watch loop four more; `CreateCluster` still wrote `member.Status` bare, the one site #96's conversion missed, and it raced the enforce pass its own `ClearExpectedIPs` spawns; seven hand-rolled `states[id] = m.Status` loops were unlocked duplicates of `memberStatesForBroadcast`, which already used the locked accessor; and `refreshLocalMonitorExpectedIPs` plus `ConfigSync`'s demotion check read the field unlocked several times each -- a pass that re-read it could take the not-Active cleanup branch and then log and act as Active. All now read one claim per pass, which fixes the pairing as well as the lock. **Unlocked `s.config` reads racing `Reconfigure`'s pointer swap:** found by this branch's own `TestConfigSyncFullConfigAdoptsTheEpochAndItsLeader`, at **9 failures in 60** under `-race` on Linux and invisible in a single run. `loadInitialMembers`, `snapshotVIPGroups` and `ConfigSync`'s tail all read `s.config` while the async reconfigure `ConfigSync` itself spawns swapped it. The callers do not agree about locking -- `Start` holds the write lock across its call and the other four do not -- so snapshotting inside would have deadlocked rather than fixed it, and `loadInitialMembers`/`snapshotVIPGroups` take the config as a parameter instead: each caller passes the snapshot it is entitled to read. 0 of 60 after, and three clean rounds of the whole `testrace` target on Linux. This is the `config.Config` ownership question (~200 unsynchronised reads, 18 unguarded writes, four disciplines) surfacing on one path; the rest of that family is untouched and still open. **`go build` does not compile test files, so use `go vet ./...`** -- three stale call sites in `vip_reconcile_test.go` passed a local build and failed CI. |
| TC-3 | #91 non-reentrancy made `ConfigSync` carry a hand-copied duplicate of the epoch compare-and-write | **Fixed 2026-09-02 (END-2339).** `adoptConvergenceMetadata` takes `s.Lock()`, so `ConfigSync`'s full-config branch — which already holds it — could not call it and carried its own copy of the compare-and-write. The method's doc comment said so outright: *"Must NOT be called with the server lock already held … ConfigSync's full-config branch adopts inline for that reason."* **This is the same non-reentrancy problem as #89's nine deadlocks, pointed sideways: instead of wedging, it duplicated.** One rule about which epoch wins, two implementations, kept in step by whoever remembered both existed — the quieter half of the same defect, and the one a deadlock detector will never find. Split into `adoptConvergenceMetadataLocked` plus the existing locking wrapper, the idiom this codebase already uses for `saveLocked`, `clusterCheckLocked`, `hasQuorumLocked` and `startReconcilePassLocked`. Equivalence was checked rather than assumed: the copy adopted on `incomingEpoch > s.clusterEpoch`, which is exactly `atLeast=false`. The `CONFIG_SYNC: Updating cluster epoch` Debug line is preserved verbatim including its `oldEpoch` field, by capturing the old value before the call, so the log surface a live run is grepped for does not move. **The larger finding is what the refactor turned up: that branch had no test coverage at all.** Flipping its `atLeast` argument from false to true — which changes which syncs may install a new leader — **passed the entire `internal/server` suite**. So the hand-copied rule was correct and nothing in the tree would have said so if it had stopped being. `TestConfigSyncFullConfigAdoptsTheEpochAndItsLeader` closes that, driving the real branch through four syncs with an advancing config stamp (the stamp must advance or the payload is rejected before reaching the adopt) and asserting the newer epoch is taken, the older refused, and — the case that matters — that an **equal** epoch does not install a different leader, which is #2's rule. Mutation-tested: `atLeast` false→true, re-inlining a divergent `>=`, and adopting unconditionally all fail it, where all three passed the whole suite before. **One test was written and then deleted rather than shipped** — a table comparing the two entry points' outcomes. Once both call one implementation it asserted only that a function agrees with its own one-line wrapper, which the compiler already guarantees, and mutation testing confirmed it caught nothing. A test shaped like coverage that has none is worse than no test, which is the same objection this branch raised against #90 |
| TC-1 | #92 nothing prevents a bare `sync.Mutex` being declared alongside the instrumented ones | **Fixed 2026-09-02 (END-2339).** #89's mechanism is worth nothing on a mutex that was never converted, so `tests/unit/lock_discipline_test.go` fails on any bare `sync.Mutex` or `sync.RWMutex` declared in `internal/`, `packages/` or `cmd/` (tests exempt; `packages/pulselock` exempt, since it wraps them). **This is deliberately the only static check, and the originally proposed one was written, run and dropped.** That checker flagged a locking method calling a locking sibling while holding the lock: over `internal/membership` and `internal/server` it produced **10 candidates — 9 false positives, and the single true positive was dead code** (`Server.PromoteNode`) — while **missing `Member.RemoveIPs → BringDownIPs`, which was live**. Seven of the nine false positives were a callee taking a *different* mutex on the same receiver (`getPeerClient → s.clientMutex`, `peerBringUpQueue → s.peerBringUpMu`, `memberStatesForBroadcast → a Member's lock`); `Server` has **nine** mutexes and the eight auxiliary ones exist *precisely so* they can be taken under the main lock. Answering that soundly needs transitive mutex-identity resolution through the call graph — a real analysis, answering statically a question the runtime answers exactly. So the static check answers the question that *is* decidable — is every mutex instrumented — and leaves "does this deadlock" to `pulselock`. It parses the AST rather than grepping, because `pulselock`'s panic strings and this codebase's own comments both contain the text `sync.Mutex` and a grep-based draft flagged them. **It ships with a second test, which matters more than it looks:** the first passes trivially if the walk finds nothing — a moved directory, a changed relative path, an early return — and then reports success having examined zero files, indistinguishable from compliance and the standard failure of every convention-checking test. `TestEveryDaemonMutexIsAccountedFor` asserts the walk still finds at least the twenty known declarations; narrowing its tree list makes it fail at "found only 0". The survey script itself is **not committed**: 1-for-10 with a live false negative is not worth keeping |
| TC-1 | #93 `internal/quorum` had no tests at all, so a reintroduced deadlock there is invisible | **Fixed 2026-09-02 (END-2339).** The package holds `QuorumManager` with an embedded `RWMutex`, `hasQuorumLocked` — one of #89's nine historical shapes — and the rule [ADR-0002](./adr/0002-two-node-availability-over-safety.md) rests on. None of it was pinned by anything. **Found by mutation, not by reading:** reintroducing the deadlock the method's own comment warns about — making `CastVote` and `concludeVotingSessionLocked` call the locking `HasQuorum` instead — changed nothing observable, because `go test ./internal/quorum/` printed *"no test files"*. An earlier pass scored that as caught because the output was not `ok`, which was wrong and is worth admitting: it was not caught, there was nothing there. **Nine tests, and two findings came out of writing them.** First, testing the lock discipline directly is not enough: taking the write lock in a test and calling `hasQuorumLocked` proves only that the method does not lock, never enters either caller, and **both mutations still escaped**. The tests that catch them drive the real path — two Yes votes on a three-node cluster records the second vote, reaches `hasQuorumLocked(yesCount)` through `canConcludeVoting`, then reaches `hasQuorumLocked(totalVotes)` inside `concludeVotingSessionLocked`, all in one write-locked region — and both now fail with `pulselock: RLock while holding Lock`, naming the shape rather than hanging. Second, **`canConcludeVoting`'s all-voted arm is redundant**, and two drafts of a test were spent discovering it: neutering `len(session.Votes) >= q.nodeCount` passes everything, and correctly, because once every node has voted `remainingPossibleYes` is 0 and the other two arms are exhaustive. No state exists where the first arm is the only one that can settle the outcome, so no test can discriminate it; the test keeps the scenario and says plainly that it does not pin the arm. **ADR-0002's rule now has a test.** Quorum is waived below three nodes so a partitioned two-node cluster can still promote and keep a Floating IP Group reachable, and the ADR notes a reader meeting that without the document "will read it as an oversight and close it" — removing it now fails `TestQuorumIsWaivedBelowThreeNodes` **by name, with the ADR path in the message**. The mirror is covered too: from three nodes up a majority is required, and half of four is not one |
| TC-6 | #94 `Member.LoadFactor` is written in 20 places and read in none | **Fixed 2026-09-02 (END-2339).** Not in the wire protocol, not marshalled (`Member` carries no JSON tags), not asserted anywhere except one test checking it had been zeroed. Twenty writes maintaining a value with no consumer — **and it was already broken, invisibly**: `SetCapacity` writes `member.Capacity` without recomputing `LoadFactor`, so the cached value went stale the moment an operator changed a node's capacity, an invariant violated in the tree and undetectable because nothing looks. The reason to remove it rather than leave it is that it was making other things harder: three of the compound writes END-2339 has to rework (`MakePassive`, `SetMode`'s consolidation, `setMaintenanceRemote`) exist to keep `Status`, `ActiveIPs` and `LoadFactor` mutually consistent under one lock. Two fields that must move together had a third bolted on, and every site touching `ActiveIPs` had to remember to recompute a derived value by hand — **the same class of constraint as the mutex discipline this ticket is about: a rule maintained by whoever remembers it.** `Member.Capacity` stays, and was nearly removed by mistake: it looked write-only once `LoadFactor`'s arithmetic went, but `member_list.go` and `health_check.go` both read `node.Capacity` off a `*Member` — they call `node.Lock()` and read `node.ActiveIPs` beside it — to build the `ipam.Node` snapshots that drive placement. **One simplification falls out**: the `BringDownIP` RPC handler carried its own copy of `RemoveActiveIPs`' filter loop, and that method's doc comment cited the copy as deliberate; the only difference was the `LoadFactor` recompute, so the handler now calls the method and the duplication goes with it. `RemoveActiveIPs` is well covered — making it a no-op, and making it keep the wrong addresses, both fail two existing tests — so collapsing onto it is safe. **Residual, deliberate:** nothing in `internal/server` failed under either mutation, so the `BringDownIP` handler's own bookkeeping has no test. The collapse is sound because the callee is covered; the caller is not |
| TC-6 | #95 a member's status and its addresses are two fields with no name for the pair | **Fixed 2026-09-02 (END-2339).** A member asserts two things about itself — its status, and the Floating IPs it says it holds — and this codebase keeps getting them wrong separately. **Every defect in the #2/#26/#58 family is one half written without the other**: a status with no addresses behind it tells peers a node is serving nothing, and addresses recorded against a member that has stopped serving keep the cluster from re-placing them. So the pair gets a name — `Claim`, defined in `CONTEXT.md`, and the word was already in the code, in `reconcileVIPPlan`'s `claim :=` and `BroadcastClusterState`'s comment about clearing a stale one. Five operations: `Claim()` reads both as one consistent pair, `SetClaim` writes both together, `UpdateClaim(decide) bool` reads-decides-writes under one acquisition, `SetActiveIPs` takes the addresses alone, `SetCapacity` is explicitly *not* part of the claim. **`Claim()` and `SetClaim()` copy the assignment set in both directions, a deliberate behaviour change**: `MakeActive` assigned the caller's slice straight into `m.ActiveIPs`, so a caller reusing its buffer could mutate a member's record **with no lock held** — the class of bug this API exists to close, reachable through its own front door. Nothing depended on the aliasing. **`UpdateClaim` is the one that needed justifying.** It hands the decision a `Claim` and takes one back, never the `*Member`, so a caller in another package cannot reach another field or take another lock — but the decision *does* run with the member lock held, and that is #85's shape exactly. `TestUpdateClaimCallingBackIntoTheMemberIsCaught` is the test that makes it acceptable: a locking call from inside the decision **panics with a `pulselock` report** rather than hanging, and its comment records the consequence — if that test ever stops panicking, `UpdateClaim` is unsafe to hand to another package and should be withdrawn. **This is why #89 had to ship before this row and not after; the ordering is load-bearing, not incidental.** `MakeActive`, `AddActiveIPs`, `RemoveActiveIPs` and `MarkUnreachable` are reimplemented on these, which is what splits the claim change from the network I/O that [ADR-0004](./adr/0004-the-lock-covers-the-state-transition.md) requires: `MakeActive`'s claim half is now literally `SetClaim`, so `SetMode`'s consolidation — which deliberately defers address work until the server lock drops — takes the first half without the second instead of writing both fields by hand. 14 tests, 8 mutations, all caught |
| TC-6 | #96 `internal/server` locks a member and edits its fields by hand at twenty sites | **Fixed 2026-09-02 (END-2339).** All twenty now go through #95's claim vocabulary; `internal/server` no longer locks a member anywhere. The grouping is the argument for having named the claim: **7 sites read only the status** (`GetStatus`), **1 read the status and the address count** (`Claim`), **4 wrote both unconditionally** (`SetClaim`), **5 wrote both conditionally** (`UpdateClaim`), **2 wrote only the addresses** (`SetActiveIPs`), **1 only the capacity** (`SetCapacity`). **Thirteen of the twenty were one of two operations** — the same two-field transition written out by hand thirteen times, each an opportunity to write one field and forget the other, which is the #2/#26/#58 family. Three sites needed care rather than translation. `setMaintenanceRemote`'s exit branch uses `SetStatus`, **not** `SetClaim`: entering maintenance clears the claim, leaving it restores eligibility and nothing else, and `SetClaim` there would clear the assignment set, discarding anything a coordinator had already given the node on its way back in — a behaviour change disguised as a tidy-up. `ConfigSync`'s peer-state loop keeps its four Debug lines inside the decision, because they take no member lock, the locked region is unchanged, and this repo verifies fixes by grepping the journal; its guard clauses become `return current, false`, which is what "ignored" has to mean. `SetMode`'s demotion reads what a node held in the same acquisition that clears it, since two acquisitions would let a concurrent assignment land between and be dropped without being released. **One regression, introduced and caught inside this row.** Converting `BroadcastClusterState`'s self-report onto `Member.GetActiveIPs` was wrong: `GetActiveIPs` returns nil for an empty set, and `ConfigSync` gates the incoming report on `senderActiveIPs != nil`, so an empty slice marshals to `[]` and means *"I hold nothing, clear my list"* while nil marshals to `null` and means *"no report, keep what you have"*. **A node that had released every address would have gone quiet about it and its peers would have kept counting those addresses as hosted — #58's shape reached by a new road.** The whole server suite passed with that in place; only reading the receiver's nil check found it. Fixed as a named function with the reasoning attached, `selfReportedAddresses`, with `self_report_test.go` pinning all three cases including the negative control that nil marshals to `null`. **It is a function rather than an inline append precisely because nothing caught the difference when it was inline** |
| TC-6 | #97 `Member` is handed across a package boundary with `Lock()` on its public surface | **Fixed 2026-09-02 (END-2339).** `Member`'s mutex becomes a named private field, so nothing outside `internal/membership` can take it and #95's claim operations are the only way in. **This is the one type END-2339 un-embeds**, and ADR-0003 records why it is the exception: `Member` is the only one handed across a boundary, and `internal/server` held its lock at twenty sites (#96). The blanket un-embed the ticket proposed was dropped because all nine of #89's deadlocks were in-package self-calls, which moving `Lock()` off a public surface does not address, and because `Server` has nine mutexes — renaming the main one to `mu` would make it indistinguishable from the eight auxiliary ones. **Two things had to be cleared first, both worth doing anyway.** `loadInitialMembers` wrote `member.Status` and `member.ActiveIPs` **with no lock at all**, across five branches, while the health check loop and the IP monitor read both — the residual #48 left open. Restructured so the branches decide and one `SetClaim` applies, which also collapses five assignments into a switch; writing a whole claim there is safe because of the guard at the top of that loop — a member already in the list is skipped with `continue`, so everything reaching the block was just created by `AddMember` and holds nothing, which matters because `loadInitialMembers` runs again after every full `ConfigSync`. It also wrote `IP`, `Port` and `Hostname` bare, so `Member` gains `SetEndpoint`/`Endpoint`: not part of the claim, but the member is already in the list by the time they are set, so a health-check pass could observe a half-described member. **The rename was driven by the compiler, not grep** — rename the field, then fix exactly what `go build` and `go vet` name, 27 passes across production and test code — because `internal/membership` has four other types whose receivers are also called `m` (`MemberList`, `HealthChecker`, `IPMonitor`) and a textual replace would have hit them. Worth recording that the first attempt at that loop silently did nothing for every `Unlock` call, because the pattern was `(Un)?Lock` and `Unlock` is `Un` plus a lowercase `lock`; it reported the same count twelve passes running. `TestMemberDoesNotExposeItsLock` pins it through `sync.Locker` rather than by reflecting over field names, since what matters is whether the method set exposes the lock — re-embedding the mutex is a one-word change that would compile and pass everything else, and now fails by name; the same test asserts the claim operations are still present, so it cannot be satisfied by deleting the surface. **Residual, with the cost measured:** `Status` and `ActiveIPs` are still *exported* fields, and **57 sites** outside `internal/membership` read or write them — mostly test scaffolding, `tests/testutils/cluster.go` alone accounting for fourteen. Unexporting them would make the compiler prove the invariant, which is the strongest version of this, and is a materially larger change than un-embedding the mutex. Follow-up ticket |
| TC-3 | #98 `config.UpdateValue` and `GetLocalNode` are unsynchronised, and `Leave`'s wipe is unguarded | **Fixed 2026-09-02 (END-2339).** The two residuals #87's fix handed forward, plus one it did not. `UpdateValue` mutated `c.Pulse`, validated, saved and rolled back on failure **with no lock at all**, while the health-check tick reaches `c.Pulse` on every pass through `GetLocalNodeUUID`; now one acquisition for the whole transaction, and it has to be one — split across separate locked calls another writer could observe or replace the intermediate state, and the rollback would restore a value that was never the one it displaced. `GetLocalNode` took and released the lock twice (`ClusterCheck`, then `GetLocalNodeUUID`) and then read `c.Nodes` unlocked, so the id came from before a concurrent join's write and the lookup ran after it. **`Validate` is split into a locking `Validate` and `validateLocked`, and this is the part worth reading:** it reads `c.Pulse` and calls `clusterCheckLocked`, so it has **always** assumed the lock was held while being named as though it took one. `Load` and `saveLocked` call it correctly from inside the lock; `UpdateValue` called it from outside. **`pulselock` caught my own mistake doing this, which is the first time #89's mechanism earned its keep on live code rather than a reconstruction:** the first draft made `Validate` locking and converted only `Load`'s call site, leaving `saveLocked` reaching it from inside the lock, and `Save` wedged immediately — panicking with a stack trace naming both frames instead of hanging the package. `TestSaveDoesNotWedgeOnValidation` keeps that shape. **A correction to what END-2339's ticket and I both claimed about `Leave`:** it was reported as writing four config fields while holding `s.RLock()`, so two concurrent `Leave` RPCs would both enter and both write. The mechanism was wrong — that `s.RLock()` is inside a `func(){...}()` whose deferred unlock fires when the closure returns, so by the writes nothing is held at all. The scanner that found it was flow-insensitive and did not track closure scope. Still a defect — four unguarded writes wiping nodes, groups and both local identifiers — but an ordinary one, the same class as the other eighteen unguarded config writes. Guarded with `s.Lock()` for local consistency, since the closure above reads `s.config.Nodes` under the server lock; scoped to the four assignments, because `Save` writes to disk and holding the server lock across a file write is #4/#8's lesson. Seven tests; the concurrency ones matter most, since what was wrong was not a wrong answer computed alone but a read or write without the lock while something else held it. Five mutations, all caught — removing either lock by `-race`, routing either caller through the locking `Validate` or `Save` by a `pulselock` panic, returning the `*Node` instead of a copy by assertion. **Residual, and it is the substance of the follow-up ticket:** about **200 unsynchronised reads** of `c.Pulse` and `c.Nodes` across the daemon and **18 more unguarded writes**, under four mutually incompatible disciplines — 18 with no lock, 12 under the server's write lock, 4 that looked like the read lock and are actually unguarded, 3 under the config's own lock. **Who owns the config's synchronisation has never been decided**, which is why all four are in the tree at once, and it has now been deferred three times: #71 synchronised the config *pointer* and left the contents, #87 deferred these two callers, and this row defers the rest. `Config` has no accessors for `c.Pulse.Mode` or `c.Pulse.HealthCheckInterval`, which is why this row's own tests read them under the lock by hand rather than adding any |
| TC-1 | #99 the cluster epoch and the elected node are read separately at twelve broadcast sites | **Fixed 2026-09-02 (END-2339).** Twelve sites passed the epoch through its locking accessor and the elected node bare, in one expression: `s.BroadcastClusterState(states, s.GetClusterEpoch()+1, s.leaderID, nil)`. `GetClusterEpoch` takes the read lock and releases it; `s.leaderID` was then read with nothing held. So the two were never read together, and an adopt landing between them pairs a new epoch with the previous one's leader. **`convergenceMetadata` already existed, returned both under one acquisition, and was used by none of the twelve.** Replaced by one method, `broadcastNextEpoch`, so the invariant lives in one place — and so the four calls sharing a scope in `performPromotionAsync` do not need four differently-named epoch variables. **The mechanism is worse than the ticket or the first diagnosis described, and the correction matters.** It was reported as installed because `GetClusterEpoch()+1` always clears the `epoch > s.clusterEpoch` guard. It does, but that guard is not what lets the stale leader through: `BroadcastClusterState` assigns `s.leaderID` **unconditionally, outside that check** — reasonable in itself, since this is the *outgoing* path where the node asserts its own view rather than the incoming one where a peer's claim must be epoch-gated. The consequence is that **a stale leaderID is installed as the elected node and broadcast to every peer as authoritative with no epoch comparison standing in its way.** The caller passing a self-consistent pair is the only protection there is. Four call sites were checked and deliberately left: `SetMode` reads both under `s.Lock()` and `broadcastConfigToPeersOnce` under `s.RLock()`, so those pairs are already atomic; `Stop` passes an empty leader on purpose; `performPromotionAsync`'s final broadcast passes an explicit epoch and target. **Confirmed a genuine data race rather than a theoretical one**: restoring the old idiom makes `TestBroadcastNextEpochDoesNotRaceAnAdopt` fail under `-race` with `WARNING: DATA RACE`. Two more mutations caught — not advancing the epoch, and broadcasting a leader other than the one read. **A test-harness gap was fixed to get there, and it explains an absence**: driving `BroadcastClusterState` panicked on a nil map, because `getPeerClient` writes to `s.peerClients` without a nil check — correct, since the real constructor initialises it at `server.go:308` — while `newConfigSyncTestServer` built a `&Server{}` literal that did not. Not a production defect, and the reason nothing in the suite had ever driven this function |
| TC-1 | #100 the deep membership check collapses seven causes into one bool, so a slow peer is latched as a foreign cluster | **Fixed 2026-09-03, VERIFIED FIXED LIVE on the docker rig — and the first fix was wrong, which the rig caught.** The deep check runs every fifth cycle and its result latches until a later deep check clears it; the latch exists so a node running its own isolated cluster stays unreachable even though it answers a TCP dial, which is real and worth keeping. But `checkClusterMembership` returned a bare `bool` over **seven** unrelated causes — an empty IP, a nil config, a missing local node id, a client that would not construct, a transport that would not connect, a gRPC `DeadlineExceeded` or `Unavailable`, a peer without us in its memberlist, and a peer echoing a different cluster token — and only the last two say anything about *which cluster* answered. So one two-second deadline on a loaded VM cost four further cycles of Unknown after the peer was answering again. **Measured on a four-node lab cluster at v4.0.2: peer frozen, first read Unknown 2.6s later, thawed, and still Unknown for a further 4.3 seconds.** That is what made an lb_api CI pipeline report a node unknown on odd occasions, and the daemon's own log line said `gRPC call to … failed` while the code comment two lines away blamed a foreign cluster. **The first fix gave the check three verdicts and latched only a rejection — and that was worse than the defect.** Part of what the latch was doing is carrying the verdict across the four cheap cycles that follow; with the unresolved verdict dropped, those cycles answer on a TCP dial alone, so a wedged-but-listening peer read healthy four cycles in five. On the three-node docker rig, a node frozen with the cgroup freezer for 14s was reported **Active for 6.2s of it** (frozen 11:27:02.8, Unknown 11:27:06.5–11:27:10.3, Active again at 11:27:10.8 *while still frozen*, thawed 11:27:16.99). A listening socket outliving a wedged daemon is #56 and #79's shape both, and the fix's own write-up claimed to protect against it while doing the opposite. **The two cases conflict under a single latch** — a transient wants fast recovery, a wedge wants sustained Unknown — so the fix separates *what was concluded* from *when to ask again*: `membershipCheckFailed` holds a rejection and latches until a later deep check confirms; `membershipUnresolved` holds "did not conclude" and keeps the node unreachable, enforced in the **cheap** branch the way a rejection already was; and `membershipRetry` earns the node **one** immediate re-check, set when it first becomes unresolved and consumed by the next pass. Decided per member, so one slow peer does not drag every other node onto the expensive path. A transient then recovers in one cycle and a wedged peer stays Unknown for as long as it stays wedged. **The second fix corrected the first, and that correction is the third measurement below.** The first version kept an unresolved node on the deep path *every* cycle, which is expensive in a way that is easy to miss: the deep check carries a 2s gRPC deadline, `performHealthChecks` holds the health checker's write lock for its whole body, and members are checked serially — so every pass stretched, and the whole cluster's health-check rate fell with it, delaying detection of *unrelated* failures. The cost applied precisely when the feature was working, since a wedged-but-listening peer is what keeps a node unresolved. **Keeping a node Unknown and re-checking it often are separable, and conflating them was the mistake:** staying Unknown needs only the cheap branch to honour the flag, which is free because that branch is a 500ms TCP dial that succeeds, and only the *first* retry has to be immediate because that is what turns five cycles of recovery into one. After that the scheduled cadence is enough — the node is already being reported Unknown, so nothing is gained by asking sooner. **Verified on the rig, all three behaviours side by side:** baseline sustained Unknown through the freeze and lagged 4.3s past the thaw; the first attempt flapped to Active for 6.2s mid-freeze; the reworked version stayed Unknown for the whole freeze and recovered on the **first sample after thaw** (49.333 against an unpause at 49.329), because of the immediate re-check. **And the cost of that re-check was measured rather than reasoned about**, one peer frozen with the cgroup freezer, identical protocol both sides: every-cycle re-checking ran at **0.40 passes/s** against a healthy baseline of 1.00, at **1 deep check per pass**; the one-shot version ran at **0.63 passes/s** at **1 deep check per 3.83 passes**, and held the peer Unknown throughout. So 58% more health-check throughput under a wedge for the same detection behaviour. **Two process failures produced a wrong answer first, and both are worth recording.** The first comparison showed *no difference at all* and very nearly went in the record as "the cost is not observable" — it was measuring the same code twice, because a branch switch with the change uncommitted let git carry the working-tree modifications across, so both builds were the same. Identical numbers were the tell and should have been chased rather than concluded from. The second run measured without checking its premise: the peer was already latched as **rejected** before being frozen, because node2 had not yet learned node1 (#103), so the unresolved path never engaged and both sides merely showed the scheduled cadence. The figures above come from runs that verify, before freezing, that the peer reads Passive and that no latch line has been logged. **Detection latency is unchanged and inherent:** a peer that stops answering gRPC while still accepting TCP is invisible until the next scheduled deep check, up to five cycles away — 2.6s on the lab, 3.7s and 6.6s on two rig runs, depending only on where in the period the freeze landed. `runUntilUnknown` in the tests names that rather than pretending it is instant; an earlier draft asserted next-cycle visibility and failed for that reason rather than a code fault. **`SIGSTOP` is not the stimulus in a container** — the daemon is PID 1 and the kernel ignores default-action signals sent to a PID namespace's init from inside it, so the first attempt at this measurement silently did nothing; `docker pause` freezes the cgroup while the kernel still completes TCP handshakes, which is exactly the wedged-but-listening shape. Tests: eleven, six driving whole cycles through a `deepCheck` seam (the same indirection idiom and the same reason as `IPMonitor`'s `enforce` and `now`; nil in the daemon) and five through the real `checkClusterMembership` against a gRPC peer answering a canned `HealthCheck`. **The real-peer tests were not optional:** the first pass tested only through the seam and two mutations escaped — removing the token-mismatch rejection and removing the not-in-memberlist rejection — meaning the entire reason the latch exists could have been deleted with every test still passing. Five mutations now caught, including one added after it escaped: **a confirmation that fails to clear `unresolved`**, which is invisible in status (the node recovers correctly but stays on the expensive path forever, so any peer that ever blipped costs five times the gRPC load for the life of the daemon) and needed a test that counts calls rather than reads status. **Residual, deliberate:** making the `Connect` failure arm return a rejection still escapes — `grpc.NewClient` is lazy, so `Connect` only errors on a malformed target and a closed port reaches the RPC-error arm instead; the arm is close to unreachable and returns the safe value |
| TC-1 | #101 `LastHCResponse` is stamped unconditionally, so it cannot measure silence and two consumers that depend on it are defeated | **Fixed 2026-09-03, VERIFIED FIXED LIVE on the docker rig.** It is the only field that can tell a blip from a corpse, and it was stamped in three places regardless of whether the member answered — so it recorded whether a member had *ever* responded, never when it last did. **Three layers, and fixing fewer than all three changes nothing.** `health_check.go` stamped it immediately *before* the branch that decides the member is unreachable, so a member was freshly stamped on the very pass that marked it Unknown. The heartbeat convergence nudge stamped every member in the snapshot every third cycle, "for consistent display", including members it had just failed to reach — its stated purpose is to keep peers aligned, and the broadcast does that, carrying statuses, the epoch and the leader but no timestamp. And `GetClusterStatus` discarded the stored value outright, reporting `time.Now()` whenever it was merely non-zero: so even with the stamps fixed, the status RPC — and with it `pulsectl`'s "Last Response" line and lb_api's status page — would still have lied. **Measured on a four-node lab cluster at v4.0.2:** a node whose daemon had been stopped for **78 seconds** reported `last_response` equal to the moment of the query, identical to the three live nodes. **Invisible through `pulsectl`**, which prints `N/A` for any Unknown node regardless of the stored value, so it shows only in the JSON/RPC that lb_api consumes — `/api/v1/status` on the appliance reported the same. **Two consumers were defeated, and the second is the more serious.** `clusterCoordinator` treats Unknown-but-within-grace as eligible, so with the grace always satisfied a permanently dead node held the coordinator role forever if it sorted lowest by id — and that role gates three early returns: the config reconcile re-broadcast, active-passive split-brain consolidation (`waiting for the coordinator to consolidate`), and the reconciliation pass. A dead node can perform none of them. `redistributeOrphanedIPs` counts an Unknown-but-within-grace node's addresses as hosted and clears them only in the arm after that, so the first arm always matched and **the clearing arm was unreachable**: addresses stranded on a permanently dead node stayed counted as hosted, `orphanedGroupIPs` never saw them, and nothing re-placed them. Its own log line, `clearing N IP(s) stranded on failed node X (silent for …)`, could never fire. That is #44's shape. `selectBestCandidate` also awarded a recency bonus every candidate always won, so it differentiated nothing. **`ACTIVE_CHECK`'s failover is unaffected and was never broken by this**, which is worth stating because it is the first thing this looks like: the check ORs the timestamp comparison with `member.Status == StatusUnknown`, and the status is what carries it — the timestamp disjunct was simply dead. **The two coordinator/reclaim consequences were reproduced separately, for attribution.** On the lab in active-active with 20 addresses at 5/5/5/5: killing a **non**-coordinator left 15 of 20 up after 60s with the victim still claiming its five, zero `stranded on failed node` lines on any live node, and the coordinator demonstrably rebalancing minutes earlier — so the coordinator works and simply never sees those addresses as orphaned. Killing the **coordinator** left 15 of 20 with **zero rebalance lines on any live node** — nobody took over. **Verified fixed on the three-node docker rig, same rig and same stimulus as its own baseline:** a node dead 75s reported `last_response` **77s ago** against the baseline's `1s ago`, and **9 of 9** addresses were up against the baseline's **6 of 9**, re-placed 5+4 across the survivors. The positive control fired: the coordinator logged `ACTIVE_CHECK: clearing 3 IP(s) stranded on failed node node2 (silent for 12s)` at a 10s grace — a line that could not previously exist. **Also fixed, one line away and undermining the same display:** the inbound `HealthCheck` handler set `Latency` to `time.Since(member.LastHCResponse)` on the line *after* stamping it to now, which is zero by construction, so an inbound check overwrote the latency our own outbound check had measured with a meaningless `~0s`. This side of the call cannot see a round trip, so it no longer writes one. Outside what the fix was asked for, and included because a truthful `lastResponse` beside a fabricated latency is half a job. Tests: eleven, with each half of the old behaviour restored as a mutation and observed to fail one by name. **Two escaped the first pass, both because the code was unreachable from the test rather than because an assertion was weak** — the heartbeat nudge is guarded by `h.server != nil` and the test's checker had none, and nothing covered `GetClusterStatus` at all; the nudge test now asserts the nudge actually fired before checking what it did, so it cannot pass by not running. The grace window's real purpose is covered on both sides: a node silent for two seconds against a ten-second grace keeps the coordinator role *and* keeps its addresses, because reclaiming on the first missed check took addresses off a node that was merely busy (#2/#26). **Residual, newly observable:** the reclaim clears only the **coordinator's** member list and the clear is not broadcast, so after it runs the coordinator reports the dead node claiming nothing while other nodes still list its addresses. Pre-existing — nothing ever cleared before, so every node agreed wrongly — and low harm here since the addresses are genuinely re-placed, but it is a divergence this fix exposes rather than creates |
| TC-6 | #102 restarting every node of an active-active cluster appears to take every floating IP down | **NOT A DEFECT — misdiagnosed, and retracted 2026-09-03 before this row was ever merged. It does not reproduce.** The observation was real: on a four-node lab appliance cluster in active-active with a 20-address group settled at 5/5/5/9, restarting all four daemons left **0 of 20 addresses up** and all four nodes Standby, with `ENFORCE: Current expectations expectations=map[]` on every node. The conclusion drawn from it — that a full restart erases the assignment map and nothing re-seeds it — was wrong. **The addresses were removed from the config, by lb_api, in the middle of the restart window.** The restarts ran between roughly 11:14 and 11:15; lb_api issued **20 `RemoveIPFromGroup` calls at 11:15:33**, over the CLI unix socket (`caller: @`, and `/var/run/pulseha/pulseha.sock` is `chown root:apache`), for exactly the 20 addresses that had been added out-of-band with `pulsectl` rather than through lb_api's own Service IP model. There was nothing left to place. **Four things point the same way.** `expectations=map[]` is what an *empty group* produces, not a lost assignment map — expectations are derived from the config and the node's interface assignment, so a populated group would list its addresses whoever was holding them. The later lab state was `Management: 1` with one node Active holding that single address, which is the system placing a one-address group correctly. **The attribution check that was run could not discriminate and was reported as though it had:** rolling all four nodes back to v4.0.2 and repeating the identical restart gave the identical 0-of-20, which was taken as proof the fixes were exonerated — true, but by then the group was already empty, so zero addresses up is the correct outcome under either hypothesis. And the three-node docker rig, which has no lb_api to reconcile anything, recovers **9 of 9** from a full-cluster restart both staggered and simultaneous, with `ACTIVE_CHECK: redistributing N orphaned floating IP(s)` logged on the coordinator each time. **What is worth keeping from this is the trap, not the defect.** On an appliance, floating IPs added directly with `pulsectl` are reaped by lb_api's periodic reconcile — measured at roughly twenty minutes here — because they are absent from its Service IP model, and lb_api wins. A test fixture built that way disappears mid-experiment and the symptom looks exactly like a PulseHA failure to place addresses. Worse, lb_api's own cluster model held only **one** node, because the cluster had been built with `pulsectl cluster create`/`join`, which bypasses lb_api entirely — so an lb_api-managed group could not have spanned the four nodes either. **Test floating IPs on an appliance must be created through lb_api, or the cluster must be built through lb_api so the two agree; otherwise use the docker rig, which has no management layer to disagree with.** Redistribution after a full restart is also quorum-gated at three or more members (`initiateIPRedistributionVote`), so a genuine failure of this shape would be expected to log `ACTIVE_CHECK: quorum vote failed, not redistributing orphaned IPs` — it logged that zero times on the rig, and that line is the thing to grep for should this ever be seen again |
| TC-3 | #103 a node that joins before the last joiner never learns about it, and only an unrelated config mutation repairs the cluster | **OPEN. Reproduced on a four-node lab cluster, 2026-09-03.** After `pulsectl cluster create` on node-1 and `cluster join` from nodes 2, 3 and 4 in that order, the four configs disagreed: node-1 and node-4 knew all four nodes, while **node-2 and node-3 each knew only three — both missing node-4**, the last joiner. Each joiner learns the membership present at the moment it joins and nothing catches it up. **The symptom is a log line once a second, indefinitely:** `CONFIG_SYNC: Member not found in member list node_id=<node-4>` on node-2, 23,000-odd occurrences in a week on a previous incarnation of the same cluster. **The mechanism is that the routine broadcasts are envelope-only.** They carry member statuses, the epoch and the leader — no config — so `ConfigSync`'s member-states loop finds the unknown member, logs, and `continue`s. Nothing in that path *adds* a member. Only a full-config push can, and the periodic full-config reconcile is **coordinator-gated**: the coordinator is the lowest node id, which in this instance was node-3 — itself missing node-4. So the incomplete view was the one being re-broadcast, while node-1, which had the complete config, was not the coordinator and never re-broadcast at all. **Two `pulsectl cluster converge` calls and 70 seconds of periodic reconcile did not repair it.** What did, as a side effect, was an unrelated config mutation: adding addresses to a group on node-1 marked the config dirty and pushed the full config, after which all four agreed. **Without such a mutation the cluster stays permanently inconsistent**, and every decision node-2 or node-3 makes over its membership — quorum counts, coordinator election, placement — is made over three nodes rather than four. **Related and distinct:** #71 synchronised the config *pointer* and left the contents; #5/#38 are about ordering between competing full-config pushes rather than a push that never happens; #43 is about a carried group being applied wholesale. This is the case where no full config is carried at all. **FIXED.** The join half does not reproduce -- on a three-node docker rig, `create` plus two sequential joins leaves all three nodes knowing all three within 25s, and the join handler does call `markConfigDirty()`, so a full config *is* broadcast. What reproduces, and what the lab actually showed, is that divergence **at an equal generation is permanent and invisible**. Constructed on the rig by removing one member from the coordinator's `config.json` and restarting it: two minutes later it was still missing that member after 133 `CONFIG_SYNC: Member not found in member list` lines, while the node holding the complete config had logged `CONFIG_RECONCILE: re-broadcasting config from the coordinator` twice. The repair broadcast is sent and refused, and the receiver names the reason -- `CONFIG_SYNC: ignoring config already superseded or held sender=... version=2 origin=...`, at exactly the two reconcile moments. Both nodes hold `version=2, origin=<the same node>`, because the diverged node received that config before its content diverged, so `configIsNewer` finds the versions equal, falls through to `incoming.origin > held.origin`, and drops it. **Content was never compared; the generation arbitrated alone.** A node that misses the one full-config broadcast for a join is not *behind* -- envelope syncs advance no generation -- so it is level and wrong, and every mechanism that could repair it declines: the receiver's guard drops what is not strictly newer (which is what makes #43's absence-means-removal safe), and `RequestConfigReconcile` re-sends without claiming a generation and is documented to skip precisely this case. That is why both `pulsectl cluster converge` runs did nothing, and why an unrelated `group add-ip` fixed it -- it minted version 3. **The fix is three parts.** A fingerprint of the shared config rides every envelope, so divergence shows in seconds; it covers what ConfigSync adopts, excludes the nine fields it preserves, and canonicalises away what is not divergence. A **majority tally** decides who is wrong: repairing merely because this node disagrees with the coordinator adopts a diverged coordinator's config exactly as a push would, spreading one wrong node across the cluster -- the failure `reconcileConfigAcrossPeers`' coordinator gate exists to prevent, and one the pull direction does nothing about, since a mismatch is symmetric. Every peer's fingerprint is already on the wire, so a node in the minority repairs itself and a node in the majority does nothing however loudly the coordinator disagrees. Two nodes can never have a majority, so that degrades to detect-and-report, for the reason in `docs/adr/0002-two-node-availability.md`. Then an **addressed pull**: `FetchConfig` returns a payload carrying `repair_for` naming the asker, which exempts it from both generation guards -- necessary, since an equal generation is the defect -- and is addressed rather than a flag so a copy delivered elsewhere is refused like any other stale config. **A false positive the tests caught and reasoning did not:** a config that has never had a plugin marshals `"plugins": null`, and the same config after a round trip marshals `{}`. A repair *is* a round trip, so the repaired node disagreed with the coordinator the instant it agreed with it, and would have pulled another repair every interval forever; `canonicalise` treats absent, null and empty as one state, with a test on the round trip itself. Observed to fail against six broken shapes: no majority gate (both a diverged coordinator overwriting a healthy peer and a two-node cluster repairing on a one-to-one disagreement), no canonicalisation, the exemption removed from either guard, the exemption not addressed, no rate limit, and envelopes without a fingerprint. **Live check:** remove a member from one node's config, restart it, and expect the cluster to repair that node within a reconcile interval with no config mutation made; then make the *coordinator* the diverged one and expect the healthy majority to keep its config and log `not in the majority`; and confirm an equal-generation payload with no `repair_for` is still refused |
...
| TC-6 | #104 `remove-ip` takes the address out of the config whether or not the release happened, so a floating IP that fails to come down can never be seen again | **Fixed 2026-09-12; the new path runs live, the refusal arm does not — found by reading the handler while diagnosing #105, not by a run.** `RemoveIPFromGroup` called `network.BringIPdown` directly, turned any failure into a warning (`// Continue anyway, as we want to remove the IP from config`), and committed the removal regardless. **That is #59 one address at a time**, and unlike a group delete it needs no unreachable peer: one misclassified netlink failure is enough, and `AddrDelSatisfied` has a live one — a prefix-length mismatch makes the kernel answer `EADDRNOTAVAIL`, which that classifier reads as *already gone* without looking. Once the address is out of every configured group nothing can see it again: `surplusFloatingIPs` scans configured groups only, and in active-passive `releaseUnassignedIPs` is gated off entirely. **The fix is #59's ordering applied to one address**: `beginIPRemoval` plans under the lock and writes nothing, `releasePlannedIPs` runs the release outside the lock through the same `BringDownIP` handler a peer would run, and `commitIPRemoval` drops the entry only once every target confirmed — the local one by reading the kernel, the one node where that is possible. An unconfirmed release returns rc=1 with the address still configured, which is the recoverable state. The old handler also made its peer gRPC calls **with `s.Lock()` held** (#4/#7/#8's shape); they now run outside it. **Two adjacent findings fixed with it.** The direct `BringIPdown` maintained neither the monitor's expectations nor `member.ActiveIPs`, and in active-passive that list is written once at promotion and never recomputed — `status` kept printing a removed address under `Active IPs` (#58's shape). And `--ip` is documented as taking an *optional* mask while the lookup defaulted a bare IPv4 address to /32 and compared strings, so `remove-ip --ip 10.0.0.2` against a configured `10.0.0.2/24` matched nothing and returned **success** — and it is the same mask mismatch that makes the kernel refuse the delete. `matchGroupIP` resolves to the configured spelling and warns; two prefixes of one address are refused rather than guessed. **What the live run does and does not show (2026-09-12, MC-LB-3-node-1).** The two-phase path is exercised — `Releasing floating IPs the config is about to stop referencing … count=1` followed by `RPC BringDownIP`, so the release now goes through the handler a peer would run rather than a bare netlink call. The arm that matters most is **not** exercised: nothing failed, so confirm-before-commit never had to refuse a removal, and the rc=1 path is covered by unit test only. The `EADDRNOTAVAIL` misclassification in `AddrDelSatisfied` is also still there — it is now caught by the kernel check before the commit rather than believed, but the classifier itself is unchanged. |
| TC-6 | #105 the post-load VIP reconcile acts on a config snapshot up to 500 ms old, so every `remove-ip` is undone a second later and the address is expected forever | **Fixed 2026-09-12, VERIFIED FIXED LIVE the same day on MC-LB-3-node-1 — and it is the defect the field report was actually about.** MC-LB-3-node-1, single-node active-passive: `pulsectl status` showed group `Management` holding one address while `ip a` and the node's `Active IPs` both showed two, `10.20.70.78/24` live on `enp1s0` with nothing in the config referencing it. **The journal shows the remove worked.** `23:48:26 Received RemoveIPFromGroup` → `Bringing down IP 10.20.70.78/24` → `Successfully brought down` → `Removed IPs from interface remaining=[10.20.70.67/24]` → `Successfully removed IP ... from group` → `ENFORCE: Current expectations expectations=map[enp1s0:[10.20.70.67/24]]`. Every step correct. **Then `23:48:27 RPC BringUpIP on iface enp1s0 for 1 IP(s)`** — `runVIPReconcile`, scheduled by the full RESYNC the appliance issues immediately *before* each CLI mutation, firing `vipReconcileDelay` later on a `vipReconcileSnapshot` captured before the remove existed. It put the address back **and** re-added it to the expectation set (`Added IPs to interface`, which also clears #60's release protection). **The snapshot was taken by the scheduler** — the comment on it claimed a pending snapshot 'describes the config the node should converge on now', which is false by up to the whole delay. It is now read by the pass, when it runs (`vipReconcilePlanNow`); the reason it was ever read early — `config.Reload()` unmarshalling over the live `*Config` — has been gone since #32 and #67 made the install a locked pointer swap. **The second half is what made one second permanent.** On a steady active-passive Active nothing recomputes the expectation set: `RefreshLocalMonitorExpectedIPs` fires only on a role transition (there are no `REFRESH:` lines in eight hours of that journal) and `Add`/`RemoveExpectedIPs` are incremental, while the enforce pass's re-derive was gated on active-active. So the bad expectation survived for the life of the daemon: at `07:06`–`07:35` every tick still logged `expectations=map[enp1s0:[10.20.70.67/24 10.20.70.78/24]]`, and at `07:28:38` an operator's `ip addr del` was answered with `IP monitor: expected IP removed from Active node; restoring` in the same second. `deriveExpectedIPs` has been mode-aware since #2/#26, so only the gate was wrong; the re-derive now runs for any Active node, and the config-derived setters still leave #60's release record alone so a release in flight is not undone. **Ungating it surfaced #87's race one level in, and CI caught it where five local `-race` runs could not.** `deriveExpectedIPs` reads `cfg.Nodes` and `cfg.Groups` with no lock, while the join handler writes `s.config.Nodes` under `s.config.Lock()` — the same shape #87 fixed for `ClusterCheck`/`GetLocalNodeUUID`, left behind because this read only ran in active-active and nothing in `./tests/unit` exercises that mode. The gate removal made it reachable and `make testrace` went red on `TestClusterInit_HandleNodeJoinBlocksDuringCreateCluster` in the first CI run. **The local suite could never have caught it**: the file is `//go:build linux`, so a macOS `-race` run does not compile it — #90's lesson arriving from the other direction. Fixed by taking the config's own lock around the reads, with `GetActiveIPs` hoisted out of the critical section so only one lock is held at a time (#89). **Positive control, run in a linux container rather than assumed: 2 of 6 runs race without the lock, 15 of 15 clean with it**, and the full `testrace` target green on linux. **The live check, 2026-09-12.** New binary installed over the Aug 18 one (backup at `/usr/local/sbin/pulseha.bak-20260912`), daemon restarted: the stale expectation was gone on the first tick (`expectations=map[enp1s0:[10.20.70.67/24]]`), and the strand then took an `ip addr del` without being restored — the same delete that was answered with `restoring` twelve hours earlier. Then the defect's own shape, three times: `cluster network resync` immediately followed by `group remove-ip`, so the remove lands inside the scheduled pass's window. **3/3 cycles the address stayed down, and there were zero `RPC BringUpIP` calls in the whole run** — that line is what the stale pass emitted at 23:48:27. **The positive control fired in the same log**, which matters because a pass that does nothing looks identical to a pass that is not running: inside the release, before the config commit, the re-derive put the expectation back (`expectations=[10.20.70.67/24 10.20.70.78/24]`) and #60's record declined it — `ENFORCE: not restoring floating IPs this node was told to release` and `IP monitor: expected IP was released on request; not restoring` — then converged to one address on the next tick. So the ungated re-derive is live, and the release-in-flight interaction it creates is the one the design expects. No warnings or errors since the restart; `10.20.70.67` served throughout bar the restart itself. **Left open beside it:** an address already stranded cannot be recovered by the daemon — nothing distinguishes it from a static address an operator put on the interface — so it needs an `ip addr del` *and* a daemon restart to clear the expectation. And `pulsectl status` now prints no `Active IPs` line at all for an election-promoted Active, rather than a stale one: that list is never populated by an election (pre-existing, called out at `seedActiveActiveAssignments`), so the honest fix is to report it from the config in active-passive. |
| TC-6 | #106 nothing can recover an address PulseHA placed and lost track of, because ownership is an allowlist | **Feature, not a defect. Added 2026-09-12, VERIFIED LIVE the same day on MC-LB-3-node-1; off by default.** #59, #104 and #105 are three handlers with one shape: the moment an address leaves the config it falls outside every set any pass computes, so a release that did not happen is permanent and the operator has to find it by hand. Each was fixed by making its own handler not lose the address — but the class stays open as long as ownership is "only touch what a group names". `pulseha.nm_address_ownership` (node-local, default false) inverts it: an address on a PulseHA-managed interface that no NetworkManager **connection profile** claims is one PulseHA placed, and is released once no group configures it. **The profile, emphatically not the device.** `nmcli device show` reports live kernel state, so it lists PulseHA's own floating IP as NetworkManager's — build the rule on that and PulseHA can never move its own VIPs again. Only `nmcli -g ipv4.addresses connection show <profile>` carries intent, and only for `method=manual`. **Four rails, each separately pinned by a test, because the failure mode is losing the node.** An interface whose profile is DHCP, unreadable or absent is skipped entirely and keeps the default behaviour — there the node's own address is absent from every profile and is indistinguishable from a strand, which is the one way this feature could take a box off the network. Every node's `bind_address` is excluded by name from the config, before NetworkManager is consulted at all, so the address the cluster runs on is not removable by an inference even where NM cannot answer. An address any group names anywhere is left to the group paths. Loopback, link-local, multicast and the unspecified address are never candidates. Candidates are computed from the config and the inventory the enforce pass already holds, and nmcli runs only if that leaves something to decide — a converged node spawns no subprocesses. Removals log at **Warn**: this deletes a live address on an inference, and that line is what an operator needs to find if the inference was ever wrong. **Found live, not by a test, and it is the reason to distrust this test suite's shape.** The first deployment did nothing at all. `IPInventory` is keyed by `ipKey`'s family-prefixed form (`4|10.0.0.1`) and the new `AddressesOn` returned the raw keys, so every address failed `net.ParseIP` in the caller and read as "cannot be a floating IP" — a silent no-op, and the unit tests all passed because **they supplied the address list themselves**. A fake at the seam below the one being tested proves nothing about the seam. `TestAddressesOnReturnsAddressesNotInventoryKeys` now builds its fixture through `ipKey` rather than from literals, so the same blind spot cannot return by changing the encoding. **The live check.** Flag off: a hand-planted `10.20.70.99/24` survived four enforce ticks untouched, zero RECLAIM lines. Flag on: both planted strands released inside one tick, each with its Warn line naming address and interface. Across the whole run `10.20.70.21` (NM's, and the bind address) and `10.20.70.67` (the configured VIP) were never touched and both still answered ICMP — and the SSH session doing the testing ran over `.21` throughout, which is the bluntest possible control. Toggling the flag back off stopped it again. **The DHCP guard, VERIFIED FIXED LIVE 2026-09-12** on the same appliance, once `enp2s0` was switched to `ipv4.method=auto` for the purpose — it took a real lease, `10.20.70.105/24`, and `nmcli -g ipv4.addresses connection show Network-2` returned **empty** against it, which is the hole stated as plainly as it can be. With a group assigned to `enp2s0` so the pass actually looked at it, the lease survived three enforce ticks with `RECLAIM: leaving an address alone reason="10.20.70.105 on enp2s0: NetworkManager's intent for this interface cannot be enumerated, so ownership is unknown"` on every one. **The negative control is what makes that worth anything**, because an interface the pass never reaches looks identical to one it decides to leave: flipping `Network-2` to `manual` with `10.20.70.105/24` declared, and planting an unclaimed `10.20.70.106/24` beside it, produced both arms in a single tick — `leaving an address alone reason="10.20.70.105 … claimed by a NetworkManager connection profile"` and `releasing an address no group and no NetworkManager profile accounts for ip=10.20.70.106`. So the pass reaches that interface, acts on it, and the DHCP skip is a decision rather than a no-op. All three rails — unknown, claimed, unclaimed — exercised on one interface against one daemon. |
| TC-6 | #107 `group unassign` releases nothing and reports success, and in active-passive no pass ever takes the addresses down | **Fixed 2026-09-12, VERIFIED FIXED LIVE the same day on MC-LB-3-node-1, both halves.** `UnassignGroupFromNode` dropped the assignment, saved, broadcast, and released nothing — its comment said "the config broadcast above lands as a ConfigSync on every peer including this node's own reconcile path, and the health checker drives VIP reconciliation off that". **Both clauses are false.** A node never ConfigSyncs itself, which vipReconciler's own comment already recorded as the asymmetry that proved #65 — so on the node the command ran on, nothing was driven at all. And the pass that would have caught it anyway, `releaseUnassignedIPs`, was gated on active-active, so in active-passive it was never called. The group's addresses stayed up on the Active node indefinitely while the operator was told the unassign had worked: **#40's operator-visible lie, returning at a different site after its own fix**, since `surplusFloatingIPs` has scanned every *configured* group rather than only the assigned ones ever since #40 and was simply never reached here. **Fixed in both places, because they answer different questions.** The handler now plans the release under the lock, drops the assignment, and releases outside it — the assignment first, which is the *opposite* order from #104 and deliberate: an Active node expects every address of every group mapped to the interface, so releasing while still assigned means the next tick brings it all back (#60), and it is `beginGroupDeletion`'s ordering for that reason. And the gate is gone, so a node that misses the explicit release converges on its own. **An unconfirmed release warns rather than refuses here, unlike #104 and #59**, and the distinction is load-bearing rather than inconsistent: the group stays *configured*, so its addresses are still referenced by something every pass can compute — configured-but-unassigned is the one state whose release pass is verified live (#58) — and an unassign that an unreachable node could block is one you cannot use when you most need it. The handler comment says so, and says not to "fix" it into a refusal without first making the addresses unreferenced. **Ungating it surfaced #87 a third time**, in `releaseUnassignedIPs`' own unlocked `cfg.Nodes`/`cfg.Groups` reads; snapshotted under the config lock like the other two, before the CI that caught the first one could. **A fourth finding, at the last layer that could have reported any of this:** `Client.UnassignGroupFromNode` discarded the response whole — warnings *and* `Success` — so every refusal the handler can return reached the operator as rc=0 and no output, including "group does not exist", "node_id not found" and "failed to save config". **The live check.** `group unassign` on the appliance took `10.20.70.67/24` down inside two seconds, logging `Releasing floating IPs the config is about to stop referencing … count=1` and `RPC BringDownIP` — and the CLI printed its success message, which it had never done. It stayed down across two ticks. The safety net was then exercised on its own by planting the address back by hand, simulating a node that missed the explicit release: one tick later, `ENFORCE: releasing floating IPs this node is no longer assigned iface=enp1s0 count=1` followed by `dropped released floating IPs from this node's assignments` — the pass that could not run in active-passive at all before this. **The two mechanisms stayed in their lanes**, which is the interaction worth checking rather than assuming: zero RECLAIM lines throughout, because #106 excludes any address a group still configures, so the surplus pass took it and the ownership pass correctly did not. Re-assigning brought the address back and the cluster returned to `online`. |
| TC-3 | #110 `log_to_syslog: false` is ignored whenever `syslog_tag` is empty, so an operator who turns syslog off still gets syslog | **Fixed 2026-09-13, VERIFIED FIXED LIVE the same day on MC-LB-3-node-1.** Two sites re-derived the setting from a proxy — `setupLogging` in `main.go` and `migrateConfig` in the config package — both reasoning "the tag is empty, so this is an old config, so syslog must be on". An empty tag is also what a *current* config looks like when nobody set one, and it is the default, so the override fired on exactly the config an operator writes when they set `log_to_syslog: false` and nothing else. **The question they were answering is real and was already answered correctly**: `New()` seeds the field true and `Load()` unmarshals the file over it, so an absent key keeps the default while a present `false` overwrites it — the only point in the program where absent and false are still distinguishable. Both re-derivations came after that point and were therefore guesses. Replaced by `Local.SyslogEnabled()`, which returns the field and carries the reasoning in its doc comment. **Found while chasing a container test failure during the Go 1.26 upgrade, not by review**, and it had been invisible for the usual reason: on any host with a working syslog the wrong decision succeeds silently, so only a machine *without* `/dev/log` could tell the two apart. **The test was the other half of the defect.** `tests/integration`'s `setupSyslogForTest` was a hand-copied duplicate of `main.go`'s logic, and its assertion branched on `cfg.Pulse.LogToSyslog` while the copy branched on the *overridden* value — so a case literally named "Syslog disabled" went down the enabled path and passed anyway. It now calls the shared decision. **The first replacement tests did not catch it either, which mutation testing caught before they shipped**: reinstating `migrateConfig`'s override left them green, because once `New()` seeds the tag that branch cannot fire at all — the config layer was never the site doing damage. The guard that works is `cmd/pulseha/main_test.go`, asserting on whether `setupLogging` *attempts* syslog rather than whether it succeeds, which discriminates on a developer machine, a CI runner and a bare container alike; reinstating the real override fails two of its three tests and leaves the positive control passing. **Live check**: with `log_to_syslog false` and an empty tag — the exact overridden shape — journald reported `{'stdout': 2}` and no syslog transport at all; re-enabling gave `{'stdout': 14, 'journal': 5, 'syslog': 48}`. The switch is real in both directions. |
| TC-6 | #108 `AddrDelSatisfied` reads `EADDRNOTAVAIL` as proof the address is gone, and a prefix-length mismatch produces exactly that with the address still up | **Fixed 2026-09-12, VERIFIED FIXED LIVE the same day on MC-LB-3-node-1.** #61 gave the release path an errno fast path — `EADDRNOTAVAIL` answered satisfied without consulting anything, because "the kernel has already answered" and the live check is a whole-host inventory on a path that fires once per address during a release storm. **The premise is wrong.** `inet_rtm_deladdr` walks the interface's addresses and skips any whose *prefix length* differs from the request's, so a delete of `10.0.0.5/32` against a held `10.0.0.5/24` matches nothing and returns `EADDRNOTAVAIL` **with the address still up**. The errno means "nothing matched this exact tuple", which is not "the address is gone" — and #104 reached the same mismatch from the config side, where a bare `--ip` was defaulted to /32 against a /24 entry. **The cost objection is answered by making the check cheap rather than by skipping it**: `BringIPdownClassified` now asks the one link (`addrHeldOnLink`, one `AddrList`) instead of `CheckIfIPExists`' every-link-both-families inventory (#64), so a storm of already-gone addresses pays *less* than #61 feared and gets a correct answer. #61's noise stays suppressed — an address that really has gone still classifies as a no-op — established by looking rather than by assuming. **The fix also recovers rather than merely reporting**: on a mismatch the delete is retried with the prefix the interface actually holds, because a floating IP is identified by its address everywhere else in this codebase (the inventory, the expectation set, `matchGroupIP`, the reclaim pass all key on the bare address). A same-prefix failure is not retried — that failed for another reason and repeating it would only log twice. **Three tests that pinned the old contract were rewritten rather than deleted**, since they asserted precisely the behaviour being removed; the replacement names the mismatch. **Live check**: config `/32` against a kernel-held `/24` built on the appliance, then `remove-ip`. On `dev` it returned **rc=0 with the address still on the interface and gone from the config** — the strand, reproduced on demand. On the fix: `WARN NETWORK: the interface holds this address under a different prefix length; retrying the delete with the one it actually has ip=10.20.70.78/32 held=10.20.70.78/24`, address down, rc=0 legitimately. |
| TC-6 | #109 the kernel confirmation #59 and #104 both rest on has never run: `GetCIDR`'s second return is the network, and the loop tested it as an error | **Fixed 2026-09-12, VERIFIED FIXED LIVE the same day — and it is the more serious of the pair.** `utils.GetCIDR` returns `(net.IP, *net.IPNet)`. `releaseIPsLocally` wrote `addr, cerr := utils.GetCIDR(ip)` and then `if cerr != nil || addr == nil { continue }` — so the `continue` fired on every address that parsed **successfully**, because the second value is non-nil exactly when parsing worked. Every candidate is in CIDR form, so every candidate was skipped, `stillHeld` was always empty, and **a release that left the address up was reported as confirmed**. That is the single guarantee #59's ordering and #104's confirm-before-commit both stand on — "the local one confirms by reading the kernel, which is the one node where that is possible" — and it was dead code from the day it was written. Two sibling sites had the same misreading (`cleanupFloatingIPsDirectly`, `cleanupFloatingIPsOnRestart`), both benign but both one edit from mattering. **Found by the live test for #108, not by review**: the `dev` build returned rc=0 for a removal that visibly left the address on the interface, which should have been impossible — #104 should have refused it. The disagreement between what the code claimed and what the appliance did is what exposed it. **The decision is now `stillHeldLocally`, extracted and tested**: that it reports an address still up, that it asks the lookup with the *bare* address (#108's mask divergence again), that a bare candidate is checked rather than skipped, and that an unparseable one is skipped rather than panicked on. An empty result is the defect, so the test says so. **Live check, and it closes #104's long-outstanding gap**: with the same address up on two interfaces so the release cannot make it absent, `remove-ip` now answers `Warning: 1 floating IP(s) of this group are still up locally on enp1s0` and `rc=1`, with the address still in the config — the recoverable state. That is #104's refusal arm, exercised live for the first time; it could never have fired before, because the check feeding it always returned nothing. |
| TC-3 | #111 inter-node gRPC is plaintext, and the TLS path that appears to address it could never have worked | **FIXED, VERIFIED FIXED LIVE run 34 (2026-09-14), all four steps. Found 2026-09-13 by review while checking Go 1.26's post-quantum TLS defaults against this tree; design accepted in [ADR-0005](adr/0005-cluster-trust-is-a-config-allowlist.md). Steps 1, 2 and 3a landed 2026-09-14 (#255, #256, #257) and were verified live as they landed; steps 3b and 4 landed in #259 and were verified on the pair the same day — see the end of this row. A cluster on the default `permissive` is still plaintext.** `internal/client.Connect` takes a `tlsEnabled` argument and **all 17 call sites pass `false`**, which reads as a flag someone forgot to flip. It is not: `false` is the only value that works. **Four things would each have to change first.** No listener serves TLS — all three are `grpc.NewServer()` with no credentials, so a client offering TLS fails at the handshake. `GenerateCertificates` is unguarded and rewrites `ca.crt` on every daemon start, observed on MC-LB-3-node-1 with the CA written at `07:55:31` against a process start of `07:55:30` — so each node has its own authority and it changes whenever that node restarts. Nothing distributes a CA, at join or anywhere else. And the client sets `InsecureSkipVerify: true`, so even a connection that completed would be encryption without identity. **So the cluster token, every `ConfigSync`, every health check and every floating-IP command cross the wire in clear**, and the scaffolding that looks like it addresses that is the reason nobody has noticed. **The proposed design is an allowlist rather than a CA**: each node holds a stable self-signed certificate, the config carries the set allowed to speak, and trust follows membership — which puts revocation on the `ConfigSync` path `#43` already made reliable, and leaves no CA key whose theft mints identities. Bootstrap is closed by extending the join token to carry the joinee's certificate fingerprint, since an operator already moves that token out of band; migration is a `permissive` phase that accumulates the trust set under plaintext, then a cluster-scoped flip to `required` that refuses unless every member is healthy and published. **Step 1: this node's certificate stops moving.** ADR-0005's first consequence, and the one everything waits on — a certificate that changes on restart cannot be in an allowlist. `EnsureCertificates` replaces the unguarded call: it reads what is on disk and regenerates only when it cannot be used, returning *why* so the server's logger can say. Unusable means any of — a file missing or unreadable, PEM that will not parse, a node certificate not signed by the CA beside it (the half-written state, since the two are written by separate renames), a certificate and key that do not match, expiry or the 30-day renewal window, or a `CommonName` that is not this host any more. Every one is a reason to regenerate rather than an error to return: what is on disk is only this node's own identity, so replacing what cannot be read costs nothing that was working. **Live, on the appliance where the defect was measured**: three restarts left `ca.crt` and `pulseha.crt` byte-identical, with the cert mtime `08:09:13` predating process starts at `08:28:38` and `08:29:09` — the exact inverse of the 07:55:31-against-07:55:30 that found this. Positive control on node-2: deleting `ca.crt` regenerated both with `Generated new TLS certificates for this node reason="CA certificate is missing"`. **Step 2: the trust set accumulates.** `config.Node` gains `tls_cert` — this node's public certificate as the node itself published it, never a key. It is the first **node-owned** field as distinct from node-local, and `ConfigSync` needed the distinction: every other field of a node entry is cluster state a peer may correct, but nobody except a node can say what its identity is, and a peer's copy is only as fresh as the last broadcast it saw. A sync therefore adopts every other node's certificate and never its own — adopting it would let a payload carrying a pre-regeneration copy hand a node back an identity it no longer has. Publishing is idempotent by comparison, since a write is a version bump plus a cluster-wide broadcast. **The live run found what the design missed**: both nodes published at boot and *neither reached the other* — node-1 converged only when an unrelated `group add-ip` happened to carry its config, and node-2 stayed unseen until a mutation of its own. "Nothing to write" and "everyone has it" are different claims and only the first is knowable locally, so the publish now also asks for a re-send (`RequestConfigReconcile` — no new generation, ignored by a peer already holding the version) and runs a beat after `Start` rather than in the same breath, for `vipReconcileDelay`'s reason: the broadcaster is still coming up. **Verified on the pair with step 1 in place**, the only configuration that means anything: from a wiped trust set, restarting both nodes and changing nothing else left each holding both certificates, byte-identical across a further restart, with the two agreeing and each node's published copy matching its own file. Run first *without* step 1, the fingerprints churned on every restart — which is why ADR-0005 ordered them. **Step 3a: the joiner's certificate travels with the join.** `JoinRequest` gains `tls_cert`, so a node is in the trust set the moment it is in the config rather than whenever its own publish propagates afterwards. Recorded, not required: a joiner on an older build sends nothing and publishes for itself a moment later, which is exactly what step 2 shipped, so an empty value is a slower path to the same place rather than something to reject. One reader — `localCertificatePEM` — serves both the join and the publish, so the two cannot drift. **Live**: with node-2 out of the cluster, node-1's trust set held only its own certificate; the instant the join returned it held node-2's as well. **Found on the way**: `getPeerClient` assigns into `s.peerClients` without checking it is non-nil, and a `Server` built as a struct literal — which several call sites and every test in the package do — has a nil map there. The assignment panics, on a goroutine the join detaches, so nothing recovers it and the daemon dies. Nothing had reached it before because no test had driven a join far enough to succeed. Now created on demand. **Step 3b deferred, and the reason is a sequencing error in ADR-0005**: the ADR has the token carry the joinee's certificate fingerprint so the joiner can pin it with a `VerifyPeerCertificate` callback — but there is no handshake to pin until step 4 puts credentials on the listener. Over a plaintext join the fingerprint would be an integrity check against a stale or wrong-cluster config, not authentication, and shipping it early would look like the latter. It belongs with step 4, where it becomes what the ADR describes — and ADR-0005 has been amended to say so, rather than leaving the register and the design document disagreeing about the order. **Step 4: the cluster can be told to require TLS.** `tls_mode` is a cluster-scoped key defaulting to permissive, because a node on a new binary has to behave exactly as the old one did until an operator says otherwise. `internal/clustertls` builds one `tls.Config` for both ends of a peer connection — every field that matters to one end is ignored by the other, both consult `VerifyPeerCertificate`, and so there is no direction in which a weaker check can be configured by accident. `RequireAnyClientCert` rather than `RequireAndVerifyClientCert`, which wants a CA pool to chain to and would reject every peer. TLS 1.3 only: both ends are this same binary. `InsecureSkipVerify` is set and is paired in the same struct with the trust-set callback — what it switches off is "did an authority vouch for this name", and what replaces it is stricter; ADR-0005's consequence has been amended to say so at the flag rather than leaving a future reader to grep it and find what looks like the original defect. **The trust set is rebuilt at each handshake, not captured when the listener is built**: a listener only rebinds when its address moves, so a captured set would refuse a node that joined afterwards and keep accepting one that had been removed — and removal-is-revocation is the property the allowlist was chosen for. `Reconfigure`'s rebind test grew a second term for the same reason, since the flip changes the listener without moving it and defect #31's fix would otherwise leave it serving plaintext to peers that had stopped speaking it. `internal/client.Connect` now takes a `*tls.Config` instead of the `tlsEnabled bool` whose only working value was `false`; the branch it replaces looked for its files at `CertDir+"pulseha.crt"`, which with no separator is not a path any file has ever been written to — proof the path had never run. Every inter-node dial goes through one `dialPeer`, so no call site can be left on the wrong side of the flip. **The flip is gated on `tlsPreconditionsMet`**: every node in the config reachable (Maintenance counts — that is a node excluded from promotion, not one that has stopped taking config) and every node holding a published certificate, with both failures reported for a node that has both so an operator does not make two trips; the trust set is then built as a last step, because a present-but-unparseable certificate passes the per-node loop. **Found by writing the test, and it is the ordering the whole thing turns on**: the flip must be pushed to the peers over a connection opened *on the terms in force before it*, before this node applies it. The first version wrote the config and let the ordinary broadcaster deliver it — which now dialled TLS, to peers still serving plaintext, so every push was refused and the one node that most needed the message was the one that could never get it. A plaintext peer in the test catches it; nothing else does. **Found again by writing the runbook, in the direction nobody tests**: an earlier version delivered *always in clear*, which is right going to `required` and catastrophic coming back — the peers are still serving TLS, a plaintext push is refused by all of them, this node reconfigures to plaintext, and the two halves each speak a protocol the other has stopped accepting, in both directions, with nothing left to repair either side. The credentials are now captured before the change is written, which makes both directions one sentence; a TLS-serving peer in the test catches the regression and the forward case still passes with plaintext, which is the discrimination that matters. A peer that still misses the push is named in the response rather than rolled back: this node and the peers that took it are consistent, and a revert would have to reach the flipped peers over a wire they have stopped accepting. **Found on the way**: `SetStructFieldByTag` compared the whole json struct tag, so a field's settability depended on whether anyone had written `,omitempty` on it — `tls_mode` was the first settable key to carry one and reported itself as an invalid config key. Tests: `internal/clustertls` drives real handshakes over a loopback socket rather than calling the callback, since `RequireAnyClientCert` is half of what is under test and a test reaching past it would pass with the listener accepting anonymous clients; mutation-tested, dropping `ClientAuth` fails the stranger-dialling-in case and capturing the trust set once fails both the revocation and the late-join cases. **Step 3b: a node can join a cluster that already requires TLS.** The ADR had this as one thing — the token carries the joinee's fingerprint, the joiner pins it — and building it found that was the easier half. The other half is that **the cluster has to let an unknown certificate in far enough to ask**, and no token can fix that from outside: a joiner is by definition not in the trust set of the cluster it is joining, so the listener refusing an unnamed certificate at the handshake made such a cluster one nobody could grow. So the two ends stop being symmetric. The dialling end verifies in full — a node never sends a request to a server the config does not name. The listening end demands a certificate and verifies nothing at the handshake, and an **authorisation interceptor** checks it against the trust set per call, letting an unnamed one reach `Join` and nothing else, still gated by the token. Encryption from the handshake, authorisation one layer up where the answer depends on what is being asked for. `RequireAnyClientCert` is what puts a certificate on the connection for the interceptor to judge; without it an anonymous caller would reach Join. On the joiner's side, `PinnedClientCredentials` checks the far end against the single fingerprint the token carried and against nothing else — deliberately not "TLS without a pin", which would be encryption to whoever answered. The stored cluster token stays a bare secret, compared byte for byte by both ends and by older binaries; the fingerprint is added on the way out of the `Token` RPC and never written down, so it is always this node's own and always current. **Found by writing the test**: a token truncated after the separator — an ordinary way for a copied string to arrive — parsed as "no fingerprint" and would have produced a silent plaintext join for an operator who had asked for a pinned one. `ParseJoinToken` now reports *whether a pin was claimed* separately from what it was, so a damaged one is refused by name instead of demoted. Tests: the whole bootstrap end to end against a real TLS listener with the real interceptor, because each piece passed on its own while joining was impossible — the handshake, the interceptor and the pin have to be right at the same time or there is no way in. Mutation-tested: not checking the pin, refusing Join at the interceptor, and dropping the fingerprint from the token each fail it. **The receiving half of the flip was untested and is now covered**: every propagation test in the package checks what a peer was *sent*, and `tls_mode` lives in the same struct as the node-local logging keys `ConfigSync` deliberately does not adopt — a `tls_mode` that ended up on that preserve list would look like a working flip on the node the operator typed on and leave every other node plaintext. The test drives a real `ConfigSync` into a node that already has a **live plaintext listener**, which is the state a node is in when the flip reaches it; a first version of it bound nothing and silently proved much less, because Reconfigure only consults the TLS term when a listener is already serving the same address, so a node with nothing bound rebinds anyway and passes whatever that term says. Mutation-tested three ways after that correction: adding `tls_mode` to the preserve list, making Reconfigure ignore the TLS term, and making the listener record ignore it each fail. Also confirmed by inspection that both gRPC services declare `Streams: []grpc.StreamDesc{}` — there are no streaming RPCs, so a unary interceptor is the whole surface and nothing bypasses authorisation. **And the mixed-build case turned out to need a guard rather than a warning.** A binary from before `tls_mode` unmarshals a config into a struct with no such field and marshals it back without one — demonstrated, not assumed — so every re-broadcast it makes deletes the key, and the coordinator makes one a minute. Read as permissive that is not one node severed, which is the visible and accepted cost of a mixed cluster, but the whole cluster quietly undoing the operator's change on a timer, over a wire it has just been told to encrypt. An absent key now means the sender has no opinion and the receiver keeps what it holds; only a key present and saying `permissive` is a flip back. That rests on permissive being written as the word and never the empty string, which is what keeps `omitempty` — and keeping it matters, because a never-flipped cluster then emits no key and hashes identically on an old binary and a new one, which is the whole of a rolling upgrade window. Mutation-tested both ways: reading an absent key as permissive fails the older-binary case, and ignoring the key entirely fails both the deliberate flip back and the ordinary propagation case. **The severest finding of the branch was the last one, and it is a cache.** There are two peer-connection caches — the server's `peerClients` pool, which carries the config and cluster-state broadcasts, and each `Member.Client`, which carries the floating-IP RPCs — and both hand back whatever they hold: the pool while its `ClientConn` is non-nil, a member while its client is non-nil. A gRPC `ClientConn` is non-nil whether or not the transport behind it works, so neither notices the cluster changed terms underneath it, and `closePeerClients` is called only at shutdown with no eviction anywhere. **Every cached connection would have stayed plaintext against peers that had just stopped accepting it, for the life of the daemon.** The listeners rebind, every log line reads correctly, `pulsectl status` looks right for a moment — and the config broadcast, the cluster-state broadcast and every floating-IP RPC are dead until a restart, which on a two-node cluster means both nodes stop hearing each other inside `fo_limit`. Demonstrated with a probe before it was fixed. `Reconfigure` now drops both caches when the terms change and only then, which matters as much as the drop: tearing working connections down on every sync is exactly defect #31's cost. Mutation-tested three ways — never dropping, always dropping, and forgetting the member cache — each failing the test that names it. **One state is detected and reported but not yet handled, and the decision is open.** If `EnsureCertificates` regenerates on a node in a `required` cluster — expiry, the 30-day renewal window, a hostname change, clock skew — that node's new certificate is in nobody's trust set, and the consequence is asymmetric: peers can still reach it, because their certificates are in *its* trust set, but it cannot reach them, because theirs refuse everything it sends except `Join` — including the `ConfigSync` that would publish the new certificate. It cannot announce its way out, its own health checks of its peers fail while theirs of it succeed, and in active-passive that is the shape that ends in a self-promotion against a healthy peer: #2/#26's duplicate addresses, reached from a new direction. `EnforceIdentityGuard` names it at startup with both fingerprints, says to re-join (the one RPC an unnamed certificate may call, and the one that records the joiner's certificate), **and holds the node out of failover promotion** by putting it into maintenance — which already means exactly this, up and reachable and never elected (`selectBestCandidate` skips it and says so). Maintenance rather than refusing to start, so the node stays diagnosable instead of being a deliberate outage. Three things it does differently from the operator's maintenance path, each of which would break it: it **cannot be refused** (`setMaintenanceLocal` declines when no other node would remain available, which is right for an operator and exactly wrong here — refusing leaves the node able to promote itself); it **does not demote over RPC** (that path calls MakePassive on peers this node cannot reach, which is the condition); and it is **not persisted** (the condition is re-derived at every start and after every join, so the guard lives exactly as long as the thing it guards against — writing `maintenance: true` would risk a node stuck in maintenance after a re-join had already fixed it, and could not propagate anyway). It releases itself once the certificate is back in the trust set, and only if it was what set the maintenance — an operator's own is never cleared by it. It does not claim to fix a node that is *already* Active and holding addresses when its identity goes stale; the realistic moment is startup, before any promotion. Mutation-tested four ways: reporting without setting maintenance, engaging regardless of `tls_mode`, releasing maintenance it did not set, and never releasing, each failing the subtest that names it. **VERIFIED LIVE 2026-09-14 on the pair, and the attempt to verify it found a worse defect first.** Deleting node-2's certificate and restarting did not engage the guard: the daemon died with `FATA … cluster TLS credentials could not be built: … open /root/.pulseha/certs/pulseha.crt: no such file or directory` and systemd restarted it into the same wall. `Start` built the cluster listener *before* `EnsureCertificates`, and on `required` the listener cannot be built without a keypair — so **a node that lost its certificate could never start again**, arriving before the code that would have replaced it. The node never got far enough to have a guard. Certificates are ensured first now, guarded by `TestCertificatesAreEnsuredBeforeTheListenerNeedsThem`, which reads the order out of `Start` and fails against the old one. With the fix, the same scenario produced the full chain: `Generated new TLS certificates … reason="node certificate is missing"`, `Serving cluster gRPC … tls=true`, `This node's certificate is not the one the cluster knows it by … onDisk=9ed64bcfc425 clusterExpects=0dfd0e5af3d6`, `Holding this node out of failover promotion …`. **The split was then observed being prevented**: node-2's own view was `Cluster Status: down`, node-1 `Unknown`, itself `Maintenance` — it genuinely could not reach node-1 (74 `Refused an RPC from a certificate the cluster does not name` on node-1 in 60s) and would have elected itself, while node-1 stayed `Active` and serving throughout. **The recovery advice was wrong and is corrected from the run**: a re-join alone is impossible, because `cluster leave` fails with `PermissionDenied … not in the cluster trust set` (leaving coordinates with the peers that are refusing it) and `cluster join` then refuses with `node is already part of a cluster; leave first`. The operator must clear the node's cluster entries from its own `config.json` and restart before joining; the log line and the runbook both say so now. Not exercised live: the guard's *self-release* path, since the repair sequence meant it never re-engaged after the fix — it is unit-tested. Mutation-tested: never reporting fails the regenerated case, and reporting regardless of mode fails the permissive case, where the node publishes its way out on the next broadcast and there is nothing to say. **Not verified live, and nothing here should be claimed until it is**: the handshake between two daemons, the `permissive`→`required` flip across a running pair, and the rolling upgrade all need two appliances, and every defect in `#104`-`#110` that mattered was caught against a real daemon with three of them invisible to the test suite. **The flip now has integration coverage, which needed one refactor to be possible at all.** Every node the integration harness runs is a `Server` in the same process, and the certificate directory was a package global — so all of them shared one identity and TLS between them could not be exercised. The directory is now a per-node value (`Server.SetCertDir`, `MemberList.SetCertDir`, threaded through `clustertls`), defaulting to `security.CertDir`, which is what the daemon runs on. `TestTheClusterCanBeFlippedToTLSWhileItIsRunning` then takes a real two-node cluster from plaintext to `required` on real sockets and checks it still converges and still propagates a config change afterwards. It catches both of the defects that had no unit-level equivalent before they were found: reverting the connection-cache fix fails it on "a config change made after the flip never reached node2", and delivering the flip on the new terms instead of the old fails it on "node2 did not accept the change". **And it found a new one immediately** — `MemberList.CertDir` took the list's read lock, and `initializeClient` reads it under paths that already hold the write lock (`AddActiveIPs` → `BringUpIPs`), so the daemon panicked on the first run; the unit suite could not see it, because nothing there drives a member all the way to dialling. It is an atomic now. The three pre-existing `NET_ADMIN` failures in that suite (`TestCombinedFeatures`, `TestGroupIPRemoval`, `TestIPMonitoring`) are unchanged and fail identically on the commit before this one. **This is not the pair**: one machine, no network, no failover under load, and `testrace` deliberately excludes the integration suite because a 1ms health-check interval does not survive the detector. **VERIFIED FIXED LIVE on the MC-LB-3 pair, 2026-09-14 (run 34)**, against `c4dea82` deployed to both nodes (`md5 51422abd…`). Baseline: healthy active-passive pair, `tls_mode` absent, both certificates already published and agreeing across the pair — node-1 `1b2f68d0…`, node-2 `39b031d5…`, each node's own file matching what both configs held. **Before**: `openssl s_client` to `:9083` on both nodes connected and got `no peer certificate available`, no cipher — plaintext, and a sharper control than a capture (the appliance has no `tcpdump`). **The flip**, `pulsectl config set tls_mode required` on node-1 at `13:19:23`, replied `applied to every node in the cluster` with no node named as missed, and produced the predicted sequence in the same second: node-1 `Cluster TLS mode changed tls_mode=required`, then **`Dropped cached peer connections after a TLS mode change pooled=1 tls=true` on *both* nodes** — the cache fix firing on exactly the one connection that would otherwise have carried plaintext forever — then `Serving cluster gRPC … tls=true` on both, node-2's arriving from the delivery that had to cross the plaintext wire the flip was removing. From `13:19:24`, `Authorised a peer against the cluster trust set node=6f1-5ed-3c6-a04 method=/rpc.Server/ConfigSync` and `/HealthCheck` in both directions. **After**: `s_client` completed a **TLS 1.3** handshake on both, each presenting exactly its own certificate (`1b2f68d0…`, `39b031d5…`); `Verify return code: 21` is correct and expected, since there is no CA to chain to — that is the design. **No failover**: roles unchanged, zero `unreachable`/`Refused`/`failed to connect` lines across six 30-second samples over three minutes, which is the check that matters because this failure is slow. **The flip back** to `permissive` was delivered over TLS — the terms the peers were still on — and both rebound to `tls=false`; without that fix node-2 would have been stranded, and this is the one direction the suite had no opinion about until the runbook was written. **Precondition**: with node-2 stopped, the flip was refused with `every node must be reachable and have published a certificate before TLS can be required (not reachable: MC-LB-3-node-2)`, rc=1. **Step 3b end to end**: on a one-node `required` cluster, `pulsectl cluster token` returned two parts whose fingerprint half was exactly node-1's own `1b2f68d089edbb41`; node-2 joined with it, logging `INITIATE_JOIN: the token pins the cluster's certificate fingerprint=1b2f68d089ed`, while node-1 logged `Accepting a join from a certificate the cluster does not name yet reason="peer certificate 39b031d5f549 is not in the cluster trust set"` — the bootstrap door opening for exactly one RPC. **Both negatives held**: a token with one hex character removed was refused with `the join token does not carry a usable certificate fingerprint (a certificate fingerprint is 64 hex characters, not 63); check it was copied whole` and did **not** fall back to a plaintext join, which is the silent downgrade the unit test found; a token naming node-2's own certificate instead of node-1's failed at the handshake with `the node being joined presented certificate 1b2f68d089ed, and the join token names 39b031d5f549 -- either the token is for a different cluster or this is not the node it names`. **Revocation**: removing node-2 produced `Refused an RPC from a certificate the cluster does not name` for `ConfigSync` and `CLI/Promote` within seconds, **with no `Serving cluster gRPC` line in that window** — the trust set is read per call, not captured at bind, which is the property the allowlist was chosen for. **The recovery procedure was exercised too, unintentionally**: removing node-2 while the cluster was `required` and then flipping node-1 back left node-2 stranded on `required`, unable even to `cluster leave` (`tls: first record does not look like a TLS handshake`) — exactly the split §9 describes; setting `tls_mode` on its config from the console and restarting restored it, and it rejoined cleanly. Cluster left as found: online, node-1 Active, node-2 Passive, `permissive`, both certificates published. **Not covered by this run**: a mixed-build rolling upgrade (the absent-key guard is tested but was not exercised here), clusters larger than two, and certificate expiry/rotation. |
| TC-6 | #112 three unreachable exports in `packages/network`, one of them a `bash -c` built by string concatenation | **Removed 2026-09-13.** `Curl`, `ICMPv4` and `ArpScan` had **zero callers** between them, in a package where every other export is on a hot path. The one worth recording is `ICMPv4`: it built ``"ping -c 1 -W 1 " + Ipv4Addr + " &> /dev/null ; echo $?"`` and handed it to `exec.Command("bash", "-c", …)`, in a daemon that runs as root. **Nothing could reach it, so nothing could exploit it** — and it is still the shape that must not be found twice, which is why the tombstone says where a shell-free liveness probe would come from instead. `Curl` and `ArpScan` shell out through `utils.Execute`, which takes a program and an argv and interpolates nothing, so those two were merely dead. **The triage is the point.** PRs #195 and #196 proposed *refactoring* `ICMPv4` to avoid the shell — the same change filed twice from two agent branches, 118 files and +22k each, and 703 commits behind `dev` before anyone read them. Neither had asked whether the function was called. That question turned an unmergeable refactor into a deletion of about thirty lines. **`TestICMPv4` went with it**, and it had been asserting nothing: it skipped in CI, and on failure it logged the error and called `t.Skip` — a test that cannot fail, guarding a function that cannot run. |
| TC-3 | #113 the cluster token is written to the log in three places, one of them at Info | **Fixed and VERIFIED FIXED LIVE 2026-09-13, both arms, on a real two-node join.** `HandleNodeJoin` logged ``Expected token: %s, Received token: %s`` at Debug, and **debug is what an operator turns on when a join will not authenticate**, so the one situation that produced the line was the one where the cluster's shared secret went to the journal and, through syslog, to wherever that is forwarded. Two more turned up on the way out, and grep found them only because the first fix made it obvious what to grep for. `CreateCluster` logged the token at **Info** — the default level, so it reached the journal on every cluster creation with nobody turning anything on — and `HealthCheck` logged both sides of a mismatch at **Warn**, repeating every tick for as long as the mismatch lasted. All three now log a `tokenFingerprint`: the first 12 hex of a SHA-256, which still answers the question the line was there for (same or not, and if not, which side changed) while carrying nothing back. An empty token is named `<empty>` rather than hashed, because "sent nothing" and "sent the wrong thing" are different problems and telling them apart is why the operator is reading the line. **Comparison is now constant-time.** `!=` on strings returns at the first differing byte, which is a remote oracle on a secret an attacker can otherwise only guess whole; `HealthCheck`'s copy is the better oracle of the two, being reachable by an unauthenticated peer at whatever rate it likes. **What was taken from PR #201 and what was not.** `TrimSpace` was taken: operators move this string by hand and a trailing newline is a right token with punctuation. `strings.EqualFold` was not — folding case throws away entropy from a secret to save an operator from a mistake nobody has been observed making — and nor was stripping a `Bearer ` prefix, which is not a paste artifact and widens what counts as valid. Tests pin both **rejections**, not just the acceptances. **The live check, once a second node existed.** MC-LB-3-node-2 was taken out of its own cluster and joined to node-1, on the fixed build, at `logging_level debug` — the exact conditions that used to write the secret out. Rejection arm first, with a deliberately wrong token: `Comparing cluster join token expected=974e91aef3fc received=3ad4ebc95112` followed by `Invalid cluster join token received`, two distinct fingerprints and the join refused. Then the acceptance arm with the real token: `CLI Join request completed success=true`. **Across the whole window, zero journal lines contained either full token** — searched for both the real one and the wrong one by literal value. An earlier single-node run could prove only the absence; it could not exercise a single new line, because one node runs no join, no create and no mismatched health check. That is why this row waited. |
| TC-3 | #114 `cluster join` with no `--bind-ip`/`--bind-port` records an address nobody can reach, and reports success | **Fixed 2026-09-14, VERIFIED FIXED LIVE the same day on the two-node lab pair.** PR #202 reported this and was closed 355 commits stale; the premise held, and the shape turned out to be three defaults in one flow, each applied by a layer that could not know the right answer. **An explicit bad address was already handled** — `preflightBind` does a real `net.Listen`, so `--bind-ip 10.20.70.99` fails with `cannot assign requested address` and rc=1. The hole was the guard reading `if req.BindIp != ""`: **the one case with no address to check was the one case not checked.** Omitting the flag sent an empty string, the joinee wrote `bind_address: ""` into `config.Nodes`, propagated it to every peer, and reported success; `pulsectl status` showed `Address: :8080  Status: Unknown` and the cluster degraded, with nothing connecting that to the join. **Fixed by deriving rather than demanding**: `localAddrToward` dials the target and reads the near end of the connection, which is the one answer that cannot disagree with reality, since peers reach this node over the path it reaches them. `preflightBind` is now unconditional — a derived address is a good guess, and that check is what turns it into a fact — and `HandleNodeJoin` refuses an empty bind address outright, validated before the member-list insertion so a refusal leaves nothing behind. **Then the live run found the second half, which no amount of reading would have.** With the address fixed the node was *still* `Unknown`: `bindPort` was hardcoded to `8080` while `targetPort` came from the operator, so `--address host:9083` produced a node listening on 8080 and recorded as 8080 — self-consistent, and unreachable on an appliance whose nftables ruleset permits 9083 with `comment "ID-pulseha"` and nothing on 8080. **And the first attempt at that fix did nothing**, because `Client.JoinClusterWithNodeID` filled in `8080` before the daemon ever saw the field empty — a third default shadowing the layer that knew, which is `#110`'s mistake one flow over. Both now defer to the daemon, which defaults to the port the cluster was reached on. **Live check**: `cluster join --address 10.20.70.21:9083 --token …` with **no other flags** recorded `10.20.70.22:9083`, came up reachable at `0.46ms` in `Maintenance` (`#115`'s documented parked state, not `Unknown`), and following the two commands it prints took the pair to `online` with node-2 Passive. |
| TC-3 | #115 a joining node is parked in maintenance on purpose, and nothing says so — the join reports success and the cluster then reports degraded | **Fixed 2026-09-13, VERIFIED FIXED LIVE the same day on a real two-node join.** `HandleNodeJoin` writes every joining node with `Maintenance: true` (`94ffa13`, 2026-05-11, "Set new nodes to maintenance mode by default"), so it cannot take floating IPs before an operator has said which ones it should serve. **That default is right and is not what was changed.** What was wrong is that it was communicated nowhere: `cluster join` printed "Successfully joined cluster", `pulsectl status` immediately reported the cluster **degraded**, no README or `docs/` page mentioned a follow-up, and the only way out — `pulsectl node maintenance --disable` — had to be discovered. **Severity, stated precisely: this is not `#105`/`#109`/`#110`'s family.** The join genuinely succeeded; the operation did what it said. It is a correct operation leaving the operator looking at a cluster that reads as broken, with no next step, at the moment they are least able to tell the difference — a communication defect, not a correctness one, and worth recording as such rather than inflated to match its neighbours. **Found by having a second node**, not by review: it took an actual join to see it, which is the same reason `#113` waited. The fix prints the state and the two commands that leave it, with the real node ID substituted so both are copy-pasteable. **Driven off the member's reported status**, not off an assumption that joins park nodes — the CLI already holds the member it just confirmed, so if the default ever changes the message stops appearing on its own instead of becoming a confident lie; a test pins that Passive, Active and Unknown all print nothing. **Live check**: node-2 left and re-joined, the message printed with its real id, and then **its own two instructions were followed verbatim** — `group assign` then `node maintenance --disable` — taking the cluster from `degraded` to `online` with node-2 Passive and eligible. Advice that is wrong is worse than none, so the advice was executed rather than read. |
