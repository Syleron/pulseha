...
| TC-6 | #102 restarting every node of an active-active cluster takes every floating IP down, and nothing re-seeds them | **OPEN. Reproduced on a four-node lab cluster, 2026-09-03.** In active-active with a 20-address group settled at **5/5/5/5**, restarting all four daemons (rolling, 12s apart) leaves **0 of 20 addresses up** and all four nodes reporting Standby. The cluster reports itself **online** and healthy throughout: `Cluster stable for 120 checks` with all four Active at sub-millisecond latency and every `last_response` fresh. What is missing is the assignment map — `ENFORCE: Current expectations expectations=map[]` on every node, once per 30s pass, indefinitely. **Pre-existing, and measured as such rather than assumed:** the first observation was on a build carrying #100 and #101, so the fixes were the obvious suspect; rolling back all four nodes to v4.0.2 (`0b4feb0`) and repeating the identical restart gave the identical result — 0 of 20, all Standby. **The cause is that the assignment map is runtime state, not config.** Each member's `ActiveIPs` lives only in the running daemon, so restarting every node at once erases the cluster's entire record of who held what, and no path re-derives it: the group is still in `floating_ip_groups` on all four nodes and still assigned to an interface on all four, but nothing computes a fresh distribution. **Neither obvious recovery works.** `pulsectl cluster converge` reports `configuration synchronized successfully` and changes nothing — it broadcasts state, not a distribution. Switching to `active-passive` elects an Active but leaves it holding almost nothing. A config *mutation* (`group add-ip`) places **only the newly added address** and leaves the pre-existing ones down. **This matters more than its symptom suggests:** an active-active cluster cannot be restarted — for an upgrade, a reboot, a maintenance window — without every floating IP going dark and staying dark until an operator intervenes, and the cluster's own status reports `online` the whole time. Distinct from #44 (an `unassign` whose reclaim never fired) and from #101 (a reclaim blocked by an always-fresh timestamp): here there is no stale claim to clear, because after a full restart nobody claims anything. **Not yet diagnosed to a line.** The candidates are `seedActiveActiveAssignments` and the reconciliation pass, both coordinator-gated, and both of which have to decide to distribute a group that no member claims — which is exactly the state `orphanedGroupIPs` should report as wholly orphaned. **Live check for any fix:** settle a group at an even share, restart every node, and expect the original count back up within a convergence window with no operator action |
