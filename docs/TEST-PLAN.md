...
| TC-3 | #103 a node that joins before the last joiner never learns about it, and only an unrelated config mutation repairs the cluster | **OPEN. Reproduced on a four-node lab cluster, 2026-09-03.** After `pulsectl cluster create` on node-1 and `cluster join` from nodes 2, 3 and 4 in that order, the four configs disagreed: node-1 and node-4 knew all four nodes, while **node-2 and node-3 each knew only three — both missing node-4**, the last joiner. Each joiner learns the membership present at the moment it joins and nothing catches it up. **The symptom is a log line once a second, indefinitely:** `CONFIG_SYNC: Member not found in member list node_id=<node-4>` on node-2, 23,000-odd occurrences in a week on a previous incarnation of the same cluster. **The mechanism is that the routine broadcasts are envelope-only.** They carry member statuses, the epoch and the leader — no config — so `ConfigSync`'s member-states loop finds the unknown member, logs, and `continue`s. Nothing in that path *adds* a member. Only a full-config push can, and the periodic full-config reconcile is **coordinator-gated**: the coordinator is the lowest node id, which in this instance was node-3 — itself missing node-4. So the incomplete view was the one being re-broadcast, while node-1, which had the complete config, was not the coordinator and never re-broadcast at all. **Two `pulsectl cluster converge` calls and 70 seconds of periodic reconcile did not repair it.** What did, as a side effect, was an unrelated config mutation: adding addresses to a group on node-1 marked the config dirty and pushed the full config, after which all four agreed. **Without such a mutation the cluster stays permanently inconsistent**, and every decision node-2 or node-3 makes over its membership — quorum counts, coordinator election, placement — is made over three nodes rather than four. **Related and distinct:** #71 synchronised the config *pointer* and left the contents; #5/#38 are about ordering between competing full-config pushes rather than a push that never happens; #43 is about a carried group being applied wholesale. This is the case where no full config is carried at all. **DIAGNOSED 2026-09-03, to a line.** The join half does not reproduce — on a three-node docker rig, `create` plus two sequential joins leaves all three nodes knowing all three within 25s, and the join handler does call `markConfigDirty()`, so a full config *is* broadcast. What reproduces, and what the lab actually showed, is the **repair** half: once a node's config has diverged, it can never be repaired. Constructed directly on the rig by removing one member from the **coordinator's** `config.json` and restarting it — the lab's exact shape, without needing to explain the original miss. Two minutes later the coordinator was still missing that member, having logged `CONFIG_SYNC: Member not found in member list` **133 times**, and the node with the complete config had logged `CONFIG_RECONCILE: re-broadcasting config from the coordinator` twice. **The repair broadcast is sent and refused.** The receiver's own line names the reason: `CONFIG_SYNC: ignoring config already superseded or held sender=… version=2 origin=…`, at exactly the two moments the reconcile fired. Both nodes hold `version=2, origin=<same node>`, because the diverged node received that very config before its content diverged — so `configIsNewer` finds the versions equal, falls through to `incoming.origin > held.origin`, gets false, and drops the payload. **Content is never compared; the stamp arbitrates alone.** So a node can hold demonstrably wrong contents at a current stamp and no re-broadcast, periodic or manual, will correct it. It also explains why `pulsectl cluster converge` did nothing on the lab twice: it re-sends the same version. And it explains what *did* repair the lab — an unrelated `group add-ip`, which minted version 3 and was therefore genuinely newer. **The candidate fix is narrow:** treat an equal-stamp payload as authoritative when it comes **from that stamp's own origin**, since the node that minted a version is the authority for its content. That changes acceptance only when the sender is the origin of the version already held, so it does not let an arbitrary peer overwrite at equal stamp. It lands in the machinery #5, #38 and #43 all live in, so it wants agreement before it is written rather than after. **Live check for any fix:** remove a member from one node's config, restart it, and expect the cluster to repair that node within a reconcile interval with no config mutation made — and separately confirm an equal-stamp payload from a *non*-origin peer is still refused |
