...
| TC-3 | #114 `cluster join` reports success when the node's bind address is not on any local interface, leaving it in the config but not in the cluster | **Open, confirmed still present on `dev` 2026-09-13 while triaging PR #202, which found it. No code changed.** Nothing between the CLI and `HandleNodeJoin` checks that `bind_address` is an address this machine actually holds — `net.Interfaces()` is consulted in `CreateCluster`, but only to build default IP groups, never to validate the bind. So a typo or a stale address produces a node that is written into `config.Nodes`, propagated to every peer by `ConfigSync`, counted in `GetMemberCount`, and reachable by nobody: the cluster is told it grew, and the operator is told it worked. **It is the same family as `#105`, `#109`, `#110` and the #246 merge — an operation reporting a success it did not achieve — and it is the one of those that an operator meets first**, since it happens at the moment they are least able to tell a working cluster from a broken one. The check is cheap and local: the joining node knows its own interfaces before it sends anything. PR #202 proposed it alongside status-output changes and a new `doctor` command, in 272 lines that were 355 commits behind by the time they were read; the validation is the valuable half and is worth redoing on its own. **Not startable as verified work on one appliance** — a join needs two nodes, like `#111` and `#113`. |
