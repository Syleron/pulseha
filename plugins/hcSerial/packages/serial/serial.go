...
// Types taken from asm-generic/termbits.h
...
// getTermios2 instantiates our struct object. Termios2 is a linux extension which allows
//             any baud rates to be set.
func getTermios2(options ConnectionOptions) (*termios2, error) {
	...
	case PARITY_EVEN:
		t2.c_cflag |= syscall.PARENB
	default:
		return nil, errors.New("invalid ParityMode value")
	}
	...
	default:
		return nil, errors.New("invalid DataBits value")
	}
...
// setup our internal connection using connection options struct object.
func setup(options ConnectionOptions) (io.ReadWriteCloser, error) {
	file, openErr :=
		os.OpenFile(
			options.PortName,
